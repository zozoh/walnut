        ...
        exec: function (str, options) {
            ...
            oReq._show_msg = function () {
                ...
                if (str) {
                    // 正常显示
                    if (oReq.status == 200) {
                        $z.invoke(options, "msgShow", [str], UI);
                    }
                    // 错误显示
                    else {
                        $z.invoke(options, "msgError", [str], UI);
                    }
                }
            };
            ...
            oReq.onreadystatechange = function () {
                ...
                if (oReq.readyState == 4) {
                    ...
                    // 最后确保通知了显示流结束
                    $z.invoke(options, "msgEnd", [str], UI);
                    
                    // 调用完成后的回调
                    var funcName = oReq.status == 200 ? "done" : "fail";
                    $z.invoke(options, funcName,   [oReq._content], UI);
                    $z.invoke(options, "complete", [oReq._content], UI);
                }
            };
