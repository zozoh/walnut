...
define(function (require, exports, module) {
  // 链入的 cSS
  seajs.use("ui/support/theme/walnut-" + window.$zui_theme + ".css"); // 声明帮助方法
  ...
  var Wn = {
    ...
    env: function env(key, dft) {
      return this.app().session.envs[key] || dft;
    },
    //...................................................................
    // 生成标准缩略图的 DOM 结构
    gen_wnobj_thumbnail_html: function gen_wnobj_thumbnail_html(nmTagName, innerOnly) {
      ...
      if (_.isString(nmTagName)) html += '<div class="wnobj-nm-con"><' + nmTagName + ' class="wnobj-nm"></' + nmTagName + '></div>'; // 进度
      ...
      html += '</div>';
      html += innerOnly ? '' : '</div>'; // 返回

      return html;
    },
    //...................................................................
    // 生成一个缩略图的 jQuery 对象，但是没加入 DOM 树
    gen_wnobj_thumbnail: function gen_wnobj_thumbnail(UI, o, nmTagName, evalThumb, nmMaxLen) {
    ...
    update_wnobj_thumbnail: function update_wnobj_thumbnail(UI, o, jq, evalThumb, nmMaxLen) {
      var jThumb = jq.find(".wnobj-thumbnail"); // 对象不存在，那么显示删除
      ...
      jq.attr("oid", o.id).attr("onm", o.nm).attr("otp", o.tp || ""); // 标记隐藏文件
      ...
      var url = o.id ? "url(/o/thumbnail/id:" + encodeURIComponent(o.id) + "?sh=64)" : "url(/o/thumbnail/type:" + encodeURIComponent(o.tp) + "?sh=64)";
      jThumb.find(".img").css("background-image", url);
      jThumb.attr("thumb", o.thumb); // 填充对象名称

      var jNm = jq.find(".wnobj-nm");

      if (jNm.length > 0) {
        var nmText = o.nm;
        if (UI && _.isFunction(UI.text)) nmText = UI.text(nmText);
        nmText = this.objDisplayName(UI, nmText, nmMaxLen);
        jNm.text(nmText); // 标记链接
    ...
    //...................................................................
    objIconHtml: function objIconHtml(o) {
      // 有了指定的 icon
      if (o.icon) return o.icon; // 主目录特殊显示
      ...
      return o.icon || '<i class="oicon ' + ('DIR' == o.race ? 'oicon-folder' : '') + '" otp="' + this.objTypeName(o) + '"></i>';
      ;
    },
    //...................................................................
    objTypeName: function objTypeName(o) {
      return o.tp || ('DIR' == o.race ? 'folder' : 'unknown');
    },
    //...................................................................
    // 本地化字符串 fnm.xxx 声明的名称，不可编辑，否则本地化失败
    isObjNameEditable: function isObjNameEditable(UI, o) {
      var nm = _.isString(o) ? o : o.nm;
      var nms = UI.msg("fnm") || {};
      return nms[nm] ? false : true;
    },
    //...................................................................
    objDisplayName: function objDisplayName(UI, nm, maxLen) {
      ...
      if (maxLen > 0) return $z.ellipsisCenter(text, maxLen);
      return text;
    },
    //...................................................................
    ...
    objDisplayPath: function objDisplayPath(UI, ph, offset, wrapper, sep) {
      var Wn = this;
      var nms = ph;
    ...
    */
    createPanel: function createPanel(o, callback, clist, opt) {
      var context = this; // 打开遮罩
    ...
    提供一个通用的文件上传界面
    */
    uploadPanel: function uploadPanel(opt) {
      var MaskUI = require("ui/mask/mask");
    ...
    */
    selectFilePanel: function selectFilePanel(opt) {
      opt = opt || {};
      ...
      new MaskUI(_.extend({
        ...
        this.$main.find(".pm-title").append($(opt.icon)).append($('<b>').text(opt.title || this.msg("selectFile")));
      });
    },
    ...
    */
    uploadToList: function uploadToList(f, opt, index) {
      opt = opt || {};
    },
    ...
    */
    __fmt_cmd_panel_opt: function __fmt_cmd_panel_opt(cmdText, maskConf, callback, referUI) {
      var opt; // 给入命令文本 
    ...
    //................................................................
    // 一个假名，用来兼容
    logpanel: function logpanel(cmdText, maskConf, callback) {
    ...
    loggingPanel: function loggingPanel(cmdText, maskConf, callback, referUI) {
      //this.logPanel(cmdText, maskConf, callback);
      // 格式化参数
      var opt = Wn.__fmt_cmd_panel_opt(cmdText, maskConf, callback);

      referUI = referUI || {}; // 显示遮罩
    ...
    processPanel: function processPanel(cmdText, maskConf, callback) {
      // 格式化参数
      var opt = Wn.__fmt_cmd_panel_opt(cmdText, maskConf, callback); // 显示遮罩
      ...
      new MaskUI(_.extend({
        ...
        var jPre = jP.find("pre"); // 收集 100% 后面的输出
        ...
        Wn.exec(opt.cmdText, {
          msgShow: function msgShow(str) {
            ...
            $('<div class="msg-info">').text(str).appendTo(jPre);
          },
          msgError: function msgError(str) {
            $('<div class="msg-err">').text(str).appendTo(jPre);
          },
          done: function done() {
            jStatus.find("h4").html(opt.maskConf.titleDone || "Done");
            jP.attr("st", "ok");
          },
          fail: function fail() {
            jStatus.find("h4").html(opt.maskConf.titleDone || "Fail");
            jP.attr("st", "fail");
          },
    ...
    //................................................................
    // 执行明后后，直接转换为json格式
    execJ: function execJ(str, input, opt) {
    ...
    //................................................................
    // 采用模板方式执行命令
    execf: function execf(tmpl, input, context, opt) {
      if (!_.isString(input)) {
        opt = context;
        context = input;
    ...
    // 如果 
    */
    exec: function exec(str, input, opt) {
      var app = window._app;
      var se = app.session;
      var re = undefined; // 没设置回调，则默认认为是同步调用
      ...
      $z.setUndefined(opt, "async", true); // 固定上下文

      var context = opt.context || this; // 没内容，那么就表执行了，直接回调吧
      ...
      var mos = "%%wn.meta." + $z.randomString(10) + "%%"; //var regex = new RegExp("^(\n"+mos+":BEGIN:)(\\w+)(.+)(\n"+mos+":END\n)$","m");

      var mosHead = "\n" + mos + ":MACRO:"; // 执行命令的地址

      var url = '/a/run/' + (appName || opt.appName || app.name); // 准备发送的数据
      ...
      oReq._show_msg = function () {
        ...
        var content = oReq.responseText; // 还没内容

        if (content.length <= oReq._last) return; //.........................................
        ...
        oReq._macro_begin_pos = content.indexOf(mosHead, oReq._last > 128 ? oReq._last - 128 : oReq._last); // 定义本次要处理的字符串
      ...
      oReq.onreadystatechange = function () {
        ...
        if (oReq._show_msg) oReq._show_msg(); // DONE: 请求结束了，调用回调
        ...
        if (oReq.readyState == 4) {
          ...
          if (macro_str) {
            ...
            while (pos_l > 0) {
              var macro = {}; // 找到行尾，作为宏类型

              var pos_begin = macro_str.indexOf('\n', pos_l);
              macro.type = $.trim(macro_str.substring(pos_l, pos_begin)); // 找到下一个宏的开始作为本宏的内容

              pos_l = macro_str.indexOf(mosHead, ++pos_begin); // 如果没有宏了，剩下的内容作为宏的内容
          ...
          for (var _i = 0, _macroArray = macroArray; _i < _macroArray.length; _i++) {
            var macro = _macroArray[_i];
            ...
            if ("update_envs" == macro.type) {
              app.session.envs = $z.fromJson(macro.content);
                ...
                var data = $z.fromJson(macro.content); // 有新回话，试图切换一下
                ...
                if (data && data.seid) {
                  $.ajax({
                    ...
                    success: function success(re) {
                      ...
                      if (reo.ok) app.session = reo.data;else $z.invoke(opt, "msgError", [re], context);
                    }
                  });
                } // 否则重定向
                else {
          ...
          if (opt.processData) {
            ...
            if (/^e[.]/.test(re)) {
              ...
              if (opt.UI) {
                ...
                re = $z.fromJson(re); // 检查是不是 session 过期了，如果过期了，直接换地址
      ...
      oReq.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); // oReq.send("cmd=" + cmdText);
      //oReq.setRequestHeader("Content-length", sendData.length);

      oReq.send(sendData); //oReq.send("mos=haha&pwd=/home/zozoh&cmd=cd")
    ...
    */
    loadAppSetup: function loadAppSetup(o, opt, callback) {
      // 格式化选项
      if (_.isFunction(opt)) {
        callback = opt;
        opt = {
      ...
      if (opt.tp) cmdText += " -tp " + opt.tp;
      Wn.exec(cmdText, function (json) {
        var asetup = $z.fromJson(json); // 强制使用指定的编辑器
        ...
        Wn.extendAppSetup(asetup, opt.forceTop, opt.inEditor); // 调用回调
    ...
    extendAppSetup: function extendAppSetup(asetup, forceTop, isInEditor) {
      var Wn = this;
      asetup.editors = asetup.editors || [];
      asetup.actions = asetup.actions || [];
      isInEditor = _.isUndefined(isInEditor) ? asetup.editors.length > 0 : isInEditor; // 首先读取编辑器

      if (isInEditor) {
        // 得到默认编辑器
        var edtnm = asetup.editors[0];
        var json = Wn.exec('app editor -cq ' + edtnm); // 出错了
        ...
        asetup.currentEditor = ace; // 如果编辑器声明了菜单，将其替换
    ...
    extendActions: function extendActions(actions, forceTop, isInEditor) {
      var UI = this; // 分析菜单项，生成各个命令的读取路径
      ...
      actions.forEach(function (str, index) {
        ...
        var ss = str.split(":"); // 在编辑器中，忽略 E

        if (isInEditor) {
          if (ss[0].indexOf("E") >= 0) return;
        } // 不在编辑器中，忽略 e
        else {
            if (ss[0].indexOf("e") >= 0) return;
          } // 生成命令的读取路径
        ...
        ac_nms.push(ss[2]); // 记录固定显示的项目下标

        if (forceTop || ss[0].indexOf("@") >= 0) menu_setup.push(index);
      }); // 逐次得到菜单的动作命令
    ...
    getChildren: function getChildren(o, filter, callback, forceReload) {
      var Wn = this; // 支持同步方式
      ...
      var do_filter = function do_filter(o, filter) {
        var reList = []; // 组合的过滤条件，预先编译
        ...
        for (var i = 0; i < o.children.length; i++) {
          ...
          var child = Wn.getById(childId); // 按照 race 过滤
      ...
      var do_after_load = function do_after_load(o, re) {
        o.children = [];
      ...
      var reList = do_filter(o, filter); // 回调
    ...
    //..............................................
    getAncestors: function getAncestors(o, includeSelf) {
      var Wn = this;
      var list = []; // 不是根，还有
    ...
    //..............................................
    getAncestorPath: function getAncestorPath(o, includeSelf) {
    ...
    //..............................................
    batchRead: function batchRead(objList, callback, context) {
      var Wn = this; // 建立一个等长的数组，用来存放返回内容

      var list = []; // 循环处理对象

      for (var i = 0; i < objList.length; i++) {
        var o = objList[i]; // 如果是字符串，可能是表示 ID 或者路径
    ...
    //..............................................
    read: function read(o, callback, context, forceReload) {
      var Wn = this;
      var store = Wn._storeAPI; // 支持同步
      ...
      var sha1Key = o.sha1 ? "SHA1:" + o.sha1 : "SHA1:!id:" + o.id; // 首选准备一个 ajax读取对象
      ...
      if (o.len == 0) return ""; // 从本地存储里读取 sha1

      var reText = store.getItem(sha1Key); // 从服务器读取
    ...
    //..............................................
    write: function write(o, content, callback, context) {
      var Wn = this;
      var store = Wn._storeAPI; // 如果是字符串，变对象
      ...
      var _save_to_cache = function _save_to_cache(re) {
        var reo = $z.fromJson(re);
    ...
    //..............................................
    get: function get(o, quiet, force) {
      if (/^id:\w{6,}$/.test(o) && o.indexOf("/") < 0) return this.getById(o.substring(3), quiet, force);
      return this.fetch(o, quiet, force);
    },
    //..............................................
    getById: function getById(oid, quiet, force) {
      if (!oid) return null;
      var Wn = this;
      var store = Wn._storeAPI; // 首先获取缓存

      var key = "oid:" + oid;
      var json = force ? null : store.getItem(key); // 有内容的话，看看是否过期了
      ...
      if (json) {
        ...
        if ((o.__local_cache || 0) + du > nowms) return o;
      } // 重新从服务器读取


      var re = Wn.exec("obj id:" + oid + " -P"); // 对于不存在的处理
    ...
    fetchBy: function fetchBy(cmdText, callback, quiet) {
      var objs; // 处理参数 
      ...
      var handler = function handler(json) {
        if (json) {
          try {
            objs = $z.fromJson(json); // for issue #224 这种随意查询的对象，先不要缓存吧，否则会爆
            // // 数组的话，循环处理对象
            // if(_.isArray(objs)){
    ...
    //..............................................
    // 得到绝对路径
    absPath: function absPath(ph) {
      // 本来就是绝对路径
      if (/^\//.test(ph)) return ph; // ~/xxxx 的路径
    ...
    //..............................................
    fetch: function fetch(ph, quiet, force) {
      var Wn = this; // 首先格式化 Path 到绝对路径
      // var ss = ph.split(/\/+/);
      // if(ss[0] == "~"){
      ...
      var aph = this.absPath(ph); // 首先试图从缓存里获取 ID

      var oid = Wn._index.ph[aph]; // 找到了 ID 就用 ID 读一下

      if (oid) return Wn.getById(oid, quiet, force); // 木有？ 那就重新根据路径加载

      var re = Wn.exec("obj '" + ph + "' -PA"); // 对于不存在的处理
    ...
    getRegularPath: function getRegularPath(o) {
      if ('DIR' == o.race) return o.ph + "/";
      return o.ph;
    },
    //..............................................
    // 本函数主要用在打开浏览对象的对话框，
    ...
    getBaseDirPath: function getBaseDirPath(o, UI, localKey) {
      // 如果是数组，那么取第一个
      if (_.isArray(o)) {
      ...
      if (_.isString(o)) o = this.get(o, true); // 试图从 local 获取上一次的记录
    ...
    //..............................................
    isInDir: function isInDir(oDir, o) {
      if (_.isString(oDir)) oDir = this.get(oDir, true);
      if (_.isString(o)) o = this.get(o, true);
      if (!oDir || !o) return false;
      var ph_dir = oDir.ph;
      var ph_obj = o.ph;
      return $z.isInPath(ph_dir, ph_obj);
    },
    //..............................................
    getRelativePath: function getRelativePath(oBase, o) {
      var ph_base = Wn.getRegularPath(oBase);
      var ph_obj = Wn.getRegularPath(o);
      return $z.getRelativePath(ph_base, ph_obj);
    },
    //..............................................
    getRelativePathToHome: function getRelativePathToHome(o) {
    ...
    //..............................................
    isHome: function isHome(o) {
      if (!o) return false;
      return o.id == this.getHome().id;
    },
    //..............................................
    // 根据 ID 列表批量获取对象的元数据
    batchGetById: function batchGetById(ids, callback, noExistsAsNull) {
      var Wn = this;
      var store = Wn._storeAPI; // 收集所有未加入缓存的项目
      ...
      var fill_unload_objs_and_invoke_callback = function fill_unload_objs_and_invoke_callback(re) {
        // 存入缓存
        var list = $z.fromJson(re);

        for (var i = 0; i < list.length; i++) {
          if (list[i]) Wn.saveToCache(list[i]);
        } // 更新未加载对象
      ...
      if (loadIds.length > 0) {
        var cmdText = "obj id:" + loadIds.join(" id:") + " -lP -json -noexists null"; // 异步调用
    ...
    //..............................................
    getObjFromCache: function getObjFromCache(oid) {
      var key = "oid:" + oid;
      var json = store.getItem(key);
      if (json) return $z.fromJson(json);
      return null;
    },
    //..............................................
    saveToCache: function saveToCache(o, cleanSubs) {
      var Wn = this;
      var store = Wn._storeAPI; // 必须要有路径
      ...
      if (oldJson) {
        var old = $z.fromJson(oldJson); // 清除子孙
      ...
      o.__local_cache = new Date().getTime(); // 存储祖先，当然本地保存的时候，就没必要保存这个字段了
    ...
    //..............................................
    removeFromCache: function removeFromCache(o) {
      var key = "oid:" + o.id;
      Wn.cleanCache(key);
    },
    //..............................................
    cleanCache: function cleanCache(key) {
      var Wn = this;
      var store = Wn._storeAPI; // 全部清除
      ...
      var _the_cleaner = function _the_cleaner(Wn, store, key, val) {
        if (!val) return; // 对象的话，要递归清理
        ...
        if (/^oid:\w{6,}/.test(key)) {
          ...
          if (_.isArray(o.children)) {
            ...
            if (o.ph && Wn._index.ph[o.ph]) delete Wn._index.ph[o.ph];
          }
        } // 移除自身
    ...
    readObj: function readObj(UI, obj, readS) {
      obj = obj || UI.app.obj; // 解析参数标志位

      var href = $z.parseHref(window.location.href);
      var params = href.params || {}; // 读元数据
    ...
    writeObj: function writeObj(UI, obj, content, readB, readS, readF) {
      obj = obj || UI.app.obj;
    ...
    // 得到系统的配置信息
    getSysConf: function getSysConf(forceReload) {
      var Wn = this;
      var store = Wn._storeAPI; // 看看缓存里有木有

      var json = forceReload ? null : store.getItem(key); // 从服务器读取

      if (!json) json = Wn.exec("sys"); // 存入缓存

      if (json) store.setItem("_WN_SYS_CONF", json); // 返回
  ...
  var store = sessionStorage; // 固定清除缓存

  store.clear(); // 从本地缓存读取所有的对象，并用 path 进行索引
  ...
  for (var i = 0; i < store.length; i++) {
    ...
    if (/^oid:\w{6,}$/.test(key)) {
      var json = store.getItem(key);
      var o = $z.fromJson(json); // 如果这个本地存储对象有效，用路径索引它的ID
