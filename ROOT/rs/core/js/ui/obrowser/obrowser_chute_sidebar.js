...
//==============================================
var html = `
<div class="ui-arena obrowser-chute-sidebar"></div>`;
//==============================================
return ZUI.def("ui.obrowser_chute_sidebar", {
    ...
    __format_sideHTML : function(jq) {
        ...
        jq.find("item").each(function(){
            ...
            // 准备提示信息
            jItem.attr("balloon", "right:"+jText.text());


            // 最后补充上 Href
            var appName = window.wn_browser_appName || "wn.browser";
            var href = "/a/open/" + appName + "?ph=" + (oid ? "id:"+oid : ph);
            if(editor)
                href += '#' + editor;
    ...
    resize: function(){
        var UI = this;
        // 显示提示
        if(UI.browser().isChuteModeMini()){
            UI.balloon("section");
        }
        // 不显示提示
        else{
            UI.balloon("section", false);
        }
    }
    //..............................................
});
