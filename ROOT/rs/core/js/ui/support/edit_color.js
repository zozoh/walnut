...
//==============================================
var html = `
<div class="ui-arena edit-color" ui-fitparent="yes">
    <div class="ec-colors"><div>
        <div class="ecc-head"><span></span></div>
        <div class="ecc-body"></div>
        <div class="ecc-foot">
            <div class="ecc-ctip"></div>
            <div class="ecc-clean">{{support.color.clean}}</div>
        </div>
    </div></div>
    <div class="ec-values">
        <div class="ecv-rgb">
            <div key="red"><b>{{support.color.red}}</b><input placeholder="0"></div>
            <div key="green"><b>{{support.color.green}}</b><input placeholder="0"></div>
            <div key="blue"><b>{{support.color.blue}}</b><input placeholder="0"></div>
        </div>
        <div class="ecv-alpha">
            <div key="alpha"><b>{{support.color.alpha}}</b><input placeholder="100"></div>
        </div>
        <div class="ecv-hex">
...
return ZUI.def("ui.support.edit_color", {
    ...
    events : {
        ...
        // 显示颜色提示
        'mouseenter .ec-colors u' : function(e){
            var UI = this;
            var jU = $(e.currentTarget);
            UI.arena.find(".ecc-ctip").text(jU.attr("value"));
        },
        'mouseleave .ec-colors' : function(e){
            this.arena.find(".ecc-ctip").text("");
        },
        // 提取颜色
        'click .ecc-body u' : function(e){
            var UI = this;
        ...
        // 清除颜色
        'click .ecc-clean' : function(){
            this.setData(null);
            this.__on_change();
        },
        // 监视 RGB 输入
        'change .ecv-rgb input' : function(e) {
            var UI = this;
        ...
        // 监视 Hex 输入
        'change .ecv-hex input' : function(e) {
            var UI = this;
            var jq = $(e.currentTarget);
            var val = jq.val();
            // 空值表示删除
        ...
        // 监视 Alpha 输入
        'change .ecv-alpha input' : function(e) {
            var UI = this;
            var jq = $(e.currentTarget);
    ...
    redraw : function() {
        ...
        with(UI.options){
            var cW    = (100 / (Zn + 1)) + "%";
            var jHead = UI.arena.find('.ecc-head>span');
            for(var i=0; i<=Zn; i++) {
    ...
    __redraw_colors : function(Z) {
        var UI = this;
        var jBody = UI.arena.find('.ecc-body').empty();
    ...
    __get_color : function(){
        var UI    = this;
        var hex   = $.trim(UI.arena.find(".ecv-hex input").val());
        var alpha = UI.__get_alpha();
        if(!hex)
            return null;
    ...
    __get_alpha : function() {
        var s = $.trim(this.arena.find(".ecv-alpha input").val());
        if("" === s) {
            return 100;
        }
    ...
    __update : function(val){
        ...
        // 设置颜色
            ...
            UI.arena.find('.ecv-rgb [key=red]   input').val(color.red)
                .attr("old-val", color.red);
            UI.arena.find('.ecv-rgb [key=green] input').val(color.green)
                .attr("old-val", color.green);
            UI.arena.find('.ecv-rgb [key=blue]  input').val(color.blue)
                .attr("old-val", color.blue);
            var alpha = parseInt(color.alpha * 100);
            UI.arena.find('.ecv-alpha input').val(alpha)
                .attr("old-val", alpha);
            var hex = color.HEX.substring(1);
            UI.arena.find('.ecv-hex input').val(hex)
                .attr("old-val", hex);
        }
    },
    //...............................................................
