...
(function () {
  ...
  var photoCompress = function photoCompress(file, conf, callback) {
    // 强制禁用
    if (conf.disable) {
      callback(file);
      return;
    ...
    if (file.type.indexOf("image") == 0 && file.size / 1024 > maxSize) {
      ...
      img.onload = function () {
        ...
        var maxWidth = conf.width || 400,
            maxHeight = conf.height || 400; // 目标尺寸

        var targetWidth = originWidth,
            targetHeight = originHeight; // 图片尺寸超过限制
        ...
        context.drawImage(img, 0, 0, targetWidth, targetHeight); // canvas转为blob
  ...
  http.photoCompress = photoCompress; //================================================ ajax请求部分

  var _ajax, _ajaxDone, _ajaxFail, _ajaxErrorMsg;

  _ajax = function _ajax(method, url, form, callback) {
  ...
  _ajaxDone = function _ajaxDone(re, callback) {
    if (http.constant.ajax.useJson) {
      if (typeof re === 'string') {
        re = eval('(' + re + ')');
      } else if (_typeof(re) == "object") {// TODO Nothing
      } else {
  ...
  _ajaxFail = function _ajaxFail(e) {
    console.log('Ajax Error!\n' + JSON.stringify(e));
  };

  _ajaxErrorMsg = function _ajaxErrorMsg(re) {
    alert(re.msg);
  };
  /**
  ...
  http.getText = function (url, form, callback) {
    ...
    var whenDone = function whenDone(text) {
  ...
  var _chkCometOpt = function _chkCometOpt(opt) {
    // 检查
    if (!opt.url) {
  ...
  var _onChange = function _onChange(respTxt, opt) {
  ...
  http.cometES = function (opt) {
    ...
    if (window.EventSource === undefined) {
      throw new Error("EventSource is not supported"); // TODO 使用xhr模拟EventSource
    }
  ...
  http.cometWS = function (opt) {// TODO
  }; // =============================================== 解析url
