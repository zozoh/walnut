...
(function () {
  ...
  var zUtil = {
    ...
    getKeyCodeValue: function getKeyCodeValue(codeName) {
      return KEY_CODE[codeName];
    },
    getKeyCodeName: function getKeyCodeName(codeValue) {
      for (var key in KEY_CODE) {
        if (KEY_CODE[key] == codeValue) return key;
      }
    },
    bodyToTop: function bodyToTop() {
      var $mybody = document.body;
      $mybody.scrollTop = 0;
    },
    bodyToBottom: function bodyToBottom() {
      var $mybody = document.body;
    ...
    // title 标题名称
    // blink 是否闪烁直到有人点了屏幕
    changeWindowTitle: function changeWindowTitle(title, blink) {
    ...
    enterFullScreen: function enterFullScreen() {
      ...
      if (_.isFunction(root.requestFullscreen)) root.requestFullscreen();else if (_.isFunction(root.mozRequestFullScreen)) root.mozRequestFullScreen();else if (_.isFunction(root.webkitRequestFullscreen)) root.webkitRequestFullscreen(Element.ALLOW_KEYBOARD_INPUT);else if (_.isFunction(root.msRequestFullscreen)) root.msRequestFullscreen();
    },
    //.............................................
    exitFullScreen: function exitFullScreen() {
      var root = document;
      if (_.isFunction(root.exitFullscreen)) root.exitFullscreen();else if (_.isFunction(root.mozCancelFullScreen)) root.mozCancelFullScreen();else if (_.isFunction(root.webkitExitFullscreen)) root.webkitExitFullscreen();else if (_.isFunction(root.msExitFullscreen)) root.msExitFullscreen();
    },
    //.............................................
    // 提供 AMD/CMD 支持功能
    defineModule: function defineModule(mdName, mdObj) {
    ...
    //.............................................
    // 安全的调用回调
    doCallback: function doCallback(callback, args, context, dftFunc) {
      // 支持 context 为函数的形式
    ...
    //.............................................
    // 处理 underscore 的模板
    tmpl: function tmpl(str, settings) {
    ...
    本函数默认占位符形式为 ${xxx}
    */
    parseTmpl: function parseTmpl(str, regex, genKey) {
      if (!regex) {
        regex = /\$\{([\s\S]+?)\}/g;

        genKey = function genKey(key) {
          return '${' + key + '}';
        };
    ...
    local: function local(appName, me, key, val) {
      if (!localStorage) throw "Browser don't support localStorage! " + appName + "-" + key;
      var appConf = $z.fromJson(localStorage.getItem(appName) || "{}");
      var myConf = appConf[me] || {}; // 设置
    ...
    getSelectedTexts: function getSelectedTexts(forceReturnArray) {
      ...
      if (sel) {
        for (var i = 0; i < sel.rangeCount; i++) {
          ...
          if (!rag.collapsed) re.push(rag.toString());
        }
      }
    ...
    strToJsObj: function strToJsObj(v, type) {
      // 指定了类型
      if (type) {
        switch (type) {
          case 'string':
    ...
    //.............................................
    toggleAttr: function toggleAttr(jq, attNm, valOn, valOff) {
      jq = $(jq); // 没有 valOn 那么，默认当做 true
      ...
      if (_.isUndefined(valOn)) {
        if (jq.attr(attNm)) {
          ...
          if (jq.attr(attNm) == valOn) {
            if (_.isUndefined(valOff)) jq.removeAttr(attNm);else jq.attr(attNm, valOff);
          } else {
            jq.attr(attNm, valOn);
          }
        } // 返回以便链式赋值
    ...
    // - base : 百分比的基数
    // @return 绝对像素数值
    dimension: function dimension(v, base) {
      // 数字
      if (_.isNumber(v)) {
        if (v > 1 || v < -1) return v;
        return v * base;
      ...
      var m = /^(-?\d*\.?\d+)(%|px|rem)?$/.exec(v); // ！靠~不知道是啥
    ...
    //.............................................
    toPixel: function toPixel(str, base, dft) {
      var re;
      var m = /^([\d.]+)(px)?(%)?$/.exec(str);

      if (m) {
        // %
        if (m[3]) return m[1] * base / 100; // 要不是 px 要不直接就是数字
    ...
    toPercent: function toPercent(n, prec, fixed) {
      prec = _.isNumber(prec) && prec >= 0 ? prec : 2;
      var m = $z.precise(n * 100, prec);
      var str = m + ""; // 用零补足

      if (fixed && prec > 0) {
        var pos = str.indexOf('.'); // 全补
    ...
    toArgs: function toArgs(list, flatten, deeply) {
      // 靠! IE 不支持，用传统方法吧
      //var args = Array.from(list);
      var args = [];
      if (list && _.isNumber(list.length)) for (var i = 0; i < list.length; i++) {
        args.push(list[i]);
      }
      if (flatten) return _.flatten(args, !deeply);
      return args;
    },
    //.............................................
    // 挑选属性，正则表达式如果以 ! 开头表示取反
    pick: function pick(obj, regex, asValueArray) {
      if (!regex) return obj; // 准备返回值

      var re = asValueArray ? [] : {}; // 如果是字符串，并且不是 ^ 开头的正则，作为半角
      // 逗号分隔的 key 列表
      ...
      if (!REG) {
        var str = $.trim(regex.toString()); // !^ 开头的正则表达式 
    ...
    //.............................................
    dump: {
      rectV: function rectV(rect) {
        return $z.tmpl("l:{{left}},r:{{right}},t:{{top}},b:{{bottom}},x:{{x}},y:{{y}}")(rect);
      },
      pos: function pos(_pos) {
        return $z.tmpl("x:{{x}},y:{{y}}")(_pos);
      }
    },
    //.............................................
    // 判断一个给定对象是否是矩形对象
    isRect: function isRect(obj) {
      return obj && _.isNumber(obj.top) && _.isNumber(obj.left) && _.isNumber(obj.width) && _.isNumber(obj.height) && _.isNumber(obj.bottom) && _.isNumber(obj.right) && _.isNumber(obj.x) && _.isNumber(obj.y);
    },
    //.............................................
    ...
    rect: function rect(ele, includeMargin, toScreen) {
      var jEle = $(ele); // 如果计算 body 或者 document 或者 window

      if (jEle[0].tagName == 'BODY') return $z.winsz(jEle[0].ownerDocument.defaultView); // 开始计算，得到相对于 document 的坐标

      var rect = jEle.offset(); // 切换到 screen 坐标系
    ...
    //.............................................
    rectObj: function rectObj(rect, keys) {
      if (_.isString(keys)) {
        keys = keys.split(/[ \t]*,[ \t]*/);
      }
    ...
    //.............................................
    rectValues: function rectValues(rect, keys) {
      if (_.isString(keys)) {
        keys = keys.split(/[ \t]*,[ \t]*/);
      }
    ...
    //.............................................
    rectDump: function rectDump(rect) {
      return this.tmpl("[{{left}},{{top}}]w={{width}},h={{height}}")(rect);
    },
    //.............................................
    // 自动根据矩形对象的值进行判断
    rect_count_auto: function rect_count_auto(rect, quiet) {
      // 有 width, height
      if (_.isNumber(rect.width) && _.isNumber(rect.height)) {
        if (_.isNumber(rect.top) && _.isNumber(rect.left)) return this.rect_count_tlwh(rect);
        if (_.isNumber(rect.bottom) && _.isNumber(rect.right)) return this.rect_count_brwh(rect);
        if (_.isNumber(rect.x) && _.isNumber(rect.x)) return this.rect_count_xywh(rect);
      } // 有 top,left
      else if (_.isNumber(rect.top) && _.isNumber(rect.left)) {
          if (_.isNumber(rect.bottom) && _.isNumber(rect.right)) return this.rect_count_tlbr(rect);
        } // 不知道咋弄了


      if (!quiet) throw "Don't know how to count rect:" + this.toJson(rect);
      return rect;
    },
    //.............................................
    // 根据 top,left,width,height 计算剩下的信息
    rect_count_tlwh: function rect_count_tlwh(rect) {
    ...
    rect_count_tlbr: function rect_count_tlbr(rect) {
    ...
    rect_count_brwh: function rect_count_brwh(rect) {
    ...
    rect_count_xywh: function rect_count_xywh(rect) {
      var W2 = rect.width / 2;
    ...
    rect_relative: function rect_relative(rect, base, forCss, baseScroll) {
      // 计算 base 的滚动
    ...
    // 返回矩形自身
    rect_zoom_tlwh: function rect_zoom_tlwh(rect, vp, zoomX, zoomY) {
      vp = vp || rect;
      zoomY = zoomY || zoomX;
    ...
    // - tY   : Y 周位移
    // 返回矩形自身
    rect_translate: function rect_translate(rect, tX, tY) {
      // 支持对象作为数据参数
    ...
    rectCss: function rectCss(rect, vpWidth, vpHeight) {
    ...
    //  - rect   : 执行停靠的矩形
    //  - target : 停靠的目标
    rect_dockIn: function rect_dockIn(rect, target, axis, padding) {
      axis = axis || {
        X: "left",
        Y: "top"
      };
      ...
      console.log(rect);
      console.log(target); // X:轴将这个大矩形移动到目标区域指定位置
    ...
    // }
    rect_compact: function rect_compact(rects, opt) {
      // 无需处理
      ...
      opt.stickRadius = opt.stickRadius || 10; // // 得到一个大矩形
      ...
      var lines = this.rect_adjustlines_create(rects, "Y", "round"); //this.rect_adjustlines_dump(lines, "after create");
      // 消除微小的线段间隔

      lines = this.rect_adjustlines_stickBy(lines, opt.stickRadius); //this.rect_adjustlines_dump(lines, "after stickBy");
      // 补偿滚动

      this.rect_adjustlines_scroll(lines, opt.scrollTop); // 寻找最小线段间隔，作为调整的单位

      var unit = this.rect_adjustlines_minSpace(lines); //console.log("UNIT:", unit);
      // 将线调整到最小间隔的倍数，最大 3 倍

      this.rect_adjustlines_compact(lines, unit, opt.maxTimes); //this.rect_adjustlines_dump(lines, "after compact");
      // 应用修改到矩形的对应字段

      this.rect_adjustlines_apply(lines); // 重新设置矩形们其他的尺寸
    ...
    // @return 从小到大排序过的线对象
    rect_adjustlines_create: function rect_adjustlines_create(rects, axis, offsetFunc) {
      // 准备 offsetFunc
      if (!_.isFunction(offsetFunc)) {
        switch (offsetFunc) {
          case "int":
            offsetFunc = function offsetFunc(v) {
              return parseInt(v);
            };
      ...
      var keys = "X" == axis ? ["left", "right"] : ["top", "bottom"]; // 准备归纳的表
    ...
    //.............................................
    // 调试打印
    rect_adjustlines_dump: function rect_adjustlines_dump(lines, title) {
      // 应用一下先
    ...
    // 并返回新的线数组
    rect_adjustlines_stickBy: function rect_adjustlines_stickBy(lines, radius) {
      var list = [];
    ...
    //.............................................
    // 统一移动各条线
    rect_adjustlines_scroll: function rect_adjustlines_scroll(lines, offset) {
      if (lines.length > 0) {
        for (var i = 0; i < lines.length; i++) {
    ...
    // 在排序后的线段组寻找调整线之间小的间距
    // @return -1 表示线段组不足两条
    rect_adjustlines_minSpace: function rect_adjustlines_minSpace(lines) {
      var re = -1;
    ...
    rect_adjustlines_compact: function rect_adjustlines_compact(lines, unit, maxTimes) {
      //console.log("-> rect_adjustlines_compact:");
      if (lines.length > 1) {
        for (var i = 1; i < lines.length; i++) {
          var lo = lines[i]; //this.rect_adjustlines_dump(lo, i);

          var times = Math.round(lo.space / unit); //console.log("times_org:", times);
          ...
          lo.offset = lines[i - 1].offset + lo.space; //this.rect_adjustlines_dump(lo, i);
        }
      }
    },
    //.............................................
    // 将调整线的 offset 重新应用到对应矩形的边
    rect_adjustlines_apply: function rect_adjustlines_apply(lines) {
      if (lines.length > 0) {
        // 更新顶点
        for (var fi in lines) {
          var lo = lines[fi];
    ...
    //.............................................
    // 计算矩形面积
    rect_area: function rect_area(rect) {
      return rect.width * rect.height;
    },
    //.............................................
    // 根据指定的 jQuery 集合，计算所有元素的最小矩形
    rect_union_by: function rect_union_by(jq) {
      var rects = [];
      jq.each(function () {
    ...
    //.............................................
    // 计算多个矩形的最小相并矩形
    rect_union: function rect_union() {
      var rects = this.toArgs(arguments, true); // 空

      if (!rects || rects.length == 0) return null; // 以第一个为基础

      var r2 = _.extend({}, rects[0]); // 只有一个


      if (rects.length == 1) return r2; // 多个
    ...
    //.............................................
    // 相并面积
    rect_union_area: function rect_union_area() {
    ...
    rect_overlap: function rect_overlap(rectA, rectB) {
      var rects = this.toArgs(arguments, true); // 少于1个

      if (!rects || rects.length <= 1) return null; // 多个
    ...
    //.............................................
    // 相交面积
    rect_overlap_area: function rect_overlap_area(rectA, rectB) {
      var rects = this.toArgs(arguments, true);
    ...
    //.............................................
    // A 是否全部包含 B
    rect_contains: function rect_contains(rectA, rectB) {
      return rectA.top <= rectB.top && rectA.bottom >= rectB.bottom && rectA.left <= rectB.left && rectA.right >= rectB.right;
    },
    //.............................................
    // 一个点是否在矩形之中，是否算上边
    rect_in: function rect_in(rect, pos, countBorder) {
      if (countBorder) {
        return rect.left <= pos.x && rect.right >= pos.x && rect.top <= pos.y && rect.bottom >= pos.y;
      }
    ...
    //.............................................
    // A 是否与 B 相交
    rect_is_overlap: function rect_is_overlap(rectA, rectB) {
      return this.rect_overlap_area(rectA, rectB) > 0;
    },
    //.............................................
    // 生成一个新的矩形
    // 用 B 限制 A，会保证 A 完全在 B 中，实在放不下了，就剪裁
    rect_clip_boundary: function rect_clip_boundary(rectA, rectB) {
      var re = {}; // @移动上下边
      // 在上面，先修改 top
    ...
    rect_move_xy: function rect_move_xy(rect, pos) {
      rect.x = pos.x;
      rect.y = pos.y;
    ...
    rect_move_tl: function rect_move_tl(rect, pos, offset) {
    ...
    */
    dock: function dock(ele, ta, opt, ignoreSetCss, vp) {
      opt = opt || {}; // 处理一下参数
      ...
      var rect = $D.rect.gen(jq, opt.viewport); //console.log($D.rect.dumpValues(rect))
      // 得到页面的矩形信息

      var viewport = $D.dom.winsz(jq[0].ownerDocument.defaultView); //console.log("viewport:", viewport);
      ...
      var mode, area, direction; // A/W/C

      if (rect.x < xx[0]) {
        area = rect.y < yy[0] ? "A" : rect.y > yy[1] ? "C" : "W";
      } // N/S
      else if (rect.x >= xx[0] && rect.x <= xx[1]) {
          area = rect.y < viewport.y ? "N" : "S";
        } // B/E/D
        else {
            area = rect.y < yy[0] ? "B" : rect.y > yy[1] ? "D" : "E";
          } // 看看是否指定了停靠模式
            ...
            area = "W";
              ...
              area = "S";
                ...
                area = "N";
                  ...
                  // 得到自己矩形的比例
                  var scale = rect.width / rect.height; // 扁的，默认停靠在水平边
                  // 窄的，默认停靠在垂直边
    ...
    // @ignoreSetCss - true 表示不强制设置 position
    dockIn: function dockIn(ele, ta, mode, ignoreSetCss) {
      var jq = $(ele);
      var jTa = $(ta);
      ...
      var rect = $z.rect(jq); //console.log(" rect  :", rect);
      // 计算页面的中点

      var viewport = $z.rect(jq.closest("body")); //console.log("viewport:", viewport);
      ...
      var mode = m ? m[1] : "H"; // 分析一下视口所在网页的区域

      var area = (m ? m[2] : null) || (viewport.x >= rect.x && viewport.y >= rect.y ? "A" : viewport.x <= rect.x && viewport.y >= rect.y ? "B" : viewport.x >= rect.x && viewport.y <= rect.y ? "C" : "D"); // 停靠在垂直边
    ...
    配合 rem 来进行浏览器适配
     ...
     - minNb : 最小值，通常为 70
    */
    do_change_root_fontSize: function do_change_root_fontSize(context, designWidth, maxNb, minNb) {
      context = context || {
        doc: document,
        win: window,
      ...
      maxNb = maxNb || 100;
      minNb = minNb || 70;
      var size = context.win.innerWidth / designWidth * maxNb; //console.log(size, Math.max(size, minNb), Math.min(Math.max(size, minNb), maxNb))
    ...
    // 查询给定元素是否有滚动条
    // 返回 {x:false, y:true}
    scrollbar: function scrollbar(jq) {
      var el = $(jq)[0];
      ...
      var ofY = cs.getPropertyValue("overflow-y"); // 有横向滚动条

      if ("scroll" == ofX || "auto" == ofX && el.clientWidth < el.scrollWidth) {
        re.x = true;
      } // 有纵向滚动条


      if ("scroll" == ofY || "auto" == ofY && el.clientHeight < el.scrollHeight) {
        re.y = true;
      } // 返回
    ...
    // 滚动到底部
    scroll2bottom: function scroll2bottom(jq) {
    ...
    doAnimatDocumentScrollTop: function doAnimatDocumentScrollTop(el, du, scrollTop) {
      // 首先找到文档
      var doc;
    ...
    //   index : 下标
    //   dft   : 如果下标越界，返回的东东
    getItem: function getItem(arr, index, dft) {
      // 从后面取重新计算一下下标
      if (index < 0) {
        index = arr.length + index;
      }

      if (index < 0 || index >= arr.length) return dft;
      return arr[index];
    },
    // 生成一个新数组，移除指定下标元素
    //   arr   : 数组
    //   index : 要移除元素下标，如果是个数组，表示多个
    removeItemAt: function removeItemAt(arr, index) {
      // 一个下标
      if (_.isNumber(index)) {
        var list = [];

        for (var i = 0; i < arr.length; i++) {
          if (i != index) list.push(arr[i]);
        }
        ...
        return list;
          ...
          for (var i = 0; i < arr.length; i++) {
            if (index.indexOf(i) < 0) list.push(arr[i]);
          }
    ...
    //   key : 键值，支持 "."
    //   dft : 如果木找到，返回的东东
    getValue: function getValue(obj, key, dft) {
      // 首先硬取试试
      var re = obj[key];
      if (!_.isUndefined(re)) return re; // 嗯，按照路径来白

      var ks = _.isArray(key) ? key : _.isString(key) ? key.split(".") : ["" + key];
      var o = obj;
    ...
    //   key : 键值，支持 "."
    //   val : 值
    setValue: function setValue(obj, key, val) {
      var ks = _.isArray(key) ? key : _.isString(key) ? key.split(".") : ["" + key];
      var o = obj;
    ...
    setMeaningful: function setMeaningful(obj, key, val) {
    ...
    isMeaningful: function isMeaningful(v) {
      if (_.isUndefined(v) || _.isNull(v)) return false;
      if (_.isNumber(v) && isNaN(v)) return false;
      if (_.isString(v) && v.length == 0) return false;
      return true;
    },
    //.............................................
    ...
    pushValue: function pushValue(obj, key, val, forceArray) {
      if (_.isUndefined(forceArray)) forceArray = true;
    ...
    addValue: function addValue(obj, key, val, forceArray) {
      var ov = obj[key]; // 加入
    ...
    mappingObj: function mappingObj(mapping, obj) {
      if (!obj) return null;
      var re = {};

      for (var key in mapping) {
        var val = mapping[key];
        var m = /^=(.+)$/.exec(val); // 直接等于某个值
    ...
    compressImageFile: function compressImageFile(file, afterCompress, iosfix, size) {
      // 旋转修正
      if (iosfix) {
    ...
    _compressAndFixImageFile: function _compressAndFixImageFile(file, afterCompress, orientation, size) {
      // 压缩图片需要的一些元素和对象
      var reader = new FileReader();
      var img = new Image(); // 缩放图片需要的canvas

      var canvas = document.createElement('canvas');
      var context = canvas.getContext('2d'); // 1,3不需要交换宽高

      var needWHChange = orientation == 6 || orientation == 8; // base64地址图片加载完毕后

      img.onload = function () {
        // 图片原始尺寸
        var originWidth = this.width;
        var originHeight = this.height;
        console.log("origin: w" + originWidth + " h" + originHeight); // 最大可接受尺寸

        var maxWidth = 0;
        var maxHeight = 0; // 无配置则默认大小一半

        if (!size) {
          maxWidth = Math.round(originWidth * 0.5);
          maxHeight = Math.round(originHeight * 0.5);
        } else {
          size = ("" + size).toLowerCase(); // 指定宽高

          if (size.indexOf("x") != -1) {
            var swh = size.split("x");
            maxWidth = parseInt(swh[0]);
            maxHeight = parseInt(swh[1]); // 交换下，因为这是在旋转前的
        ...
        var targetWidth = originWidth,
            targetHeight = originHeight; // 图片尺寸超过最大图片限制
        ...
        context.drawImage(img, 0, 0, targetWidth, targetHeight); // 是否需要选择
        ...
        if (orientation != 1) {
          switch (orientation) {
            case 6:
              ...
              context.rotate(Math.PI / 2); // (0,-imgHeight) 从旋转原理图那里获得的起始点
    ...
    uploadFile: function uploadFile(opt) {
      // 没必要上传
      if (!opt.file) throw "!!! without field : 'file' "; // 开始上传

      var xhr = new XMLHttpRequest(); // 检查
      ...
      if ("ajax" == opt.evalReturn) {
        opt.evalReturn = function (xhr) {
          ...
          opt.evalReturn = function (xhr) {
            return {
              re: xhr,
              status: xhr.status == 200 ? "done" : "fail"
            };
          };
        } // 进度回调


      if (typeof opt.progress == "function") xhr.upload.addEventListener("progress", opt.progress, false); // 完成的处理

      xhr.onreadystatechange = function (e) {
        if (xhr.readyState == 4) {
          var r = opt.evalReturn(xhr);
          $z.invoke(opt, r.status, [r.re]); // 统一处理完成
      ...
      var url = $z.tmpl(opt.url)(opt); //console.log("upload to:", url);

      xhr.open("POST", url, true);
      xhr.setRequestHeader('Content-type', "application/x-www-form-urlencoded; charset=utf-8"); // 修改提示图标的标签

      $z.invoke(opt, "beforeSend", [xhr]); // 执行上传

      xhr.send(opt.file);
    },
    // 对于 AJAX 请求的返回对象，进行检查，如果发现是过期 session 报的错，直接踢回登录页面
    checkSessionNoExists: function checkSessionNoExists(re, loginUrl) {
    ...
    //  - text:///path/to/text
    //  - jso:///path/to/js
    loadResource: function loadResource(rs, callback, context) {
      var ME = this; // 对结果的处理函数

      var _eval_re = function _eval_re(type, re) {
        // 根据特定的类型处理数据
        if ("json" == type) {
      ...
      if (_.isString(rs)) {
        ...
        if (m) {
          var reObj;
          var type = m[1];
          var url = m[2]; // 看看缓冲里有木有

          context = context || this;
          var str = ME._app_rs[rs]; // 缓冲里有，那么就不用请求
          ...
          var async = _.isFunction(callback); // console.log("async:", async)
          ...
          var ajaxConf = {
            ...
            success: function success(re) {
              // 计入缓存
              ME._app_rs[rs] = re; //console.log("success:", re);
              // 根据特定的类型处理数据

              reObj = _eval_re(type, re); // 判断是否是过期

              ME.checkSessionNoExists(reObj); // 解开所有嵌套资源

              reObj = ME.unwrapObjResource(reObj); // 调用回调

              if (_.isFunction(callback)) {
                callback.call(context, ME.extend({}, reObj));
              }
            },
            error: function error(re, reason) {
              alert("fail to load resource: " + rs + " : because " + reason);
            }
          }; // 发送请求

          $.ajax(url, ajaxConf); // 返回
    ...
    unwrapObjResource: function unwrapObjResource(obj) {
      var ME = this; // 数组
    ...
    // __dis_obj 方法
    evalFldDisplay: function evalFldDisplay(fld) {
      var func; // 自定义的 display 方法
      ...
      if (fld.display) {
        ...
        if (_.isFunction(fld.display)) {
          ...
          // 预编译 icon
          if (fld.icon) fld.__dis_icon = _.isFunction(fld.icon) ? fld.icon : $z.tmpl(fld.icon); // 预编译 text

          if (fld.text) fld.__dis_text = _.isFunction(fld.text) ? fld.text : $z.tmpl(fld.text); // 同时有 icon 和 text

          if (fld.__dis_icon && fld.__dis_text) {
            func = function func(o, jso) {
              var fld = _.isFunction(jso.type) ? jso.type() : jso;
              return fld.__dis_icon.call(this, o, jso) + fld.__dis_text.call(this, o, jso);
            };
          } // 只有 icon
              ...
              func = fld.__dis_icon;
                ...
                func = fld.__dis_text;
                  ...
                  func = function func(o, jso, UI) {
                    // 指定了类型转换，以及 UI
                    if (jso && jso.__jso_type && UI) return UI.text(jso.parseByObj(o).toText()); // 否则直接变字符串
    ...
    evalData: function evalData(data, params, callback, context) {
      if (!data) {
      ...
      var async = true; // 如果回调不是函数，那么将其视为 context，同时这必定是一个同步调用
      // 那么这里会设置返回值

      if (!_.isFunction(callback)) {
        context = context || callback;
        async = false;
        callback = null; // callback = function (objs) {
        //     eval_re = objs;
        // };
      } // 确保有 context


      context = context || this; // 数组
      ...
      if (_.isArray(data)) {
        callback.apply(context, [data]);
          ...
          if (!_.isUndefined(eval_re)) {
            ...
            var str = $z.tmpl(data)(params || {}); //console.log(">> exec: ", str)
    ...
    evalObjValue: function evalObjValue(val, params, context) {
      if (_.isFunction(val)) {
    ...
    offsetMonthly: function offsetMonthly(d, off) {
    ...
    // @return 偏移后的日期对象，如果你传入的是日期对象会是同一份实例
    offsetWeekly: function offsetWeekly(d, off) {
      var d2 = this.parseDate(d);
      var ms = d2.getTime();
      d2.setTime(ms + 86400000 * 7 * off);
      return d2;
    },
    getWeekOfYear: function getWeekOfYear(d) {
     ...
     */
    parseDate: function parseDate(str, regex) {
      if (!str) {
        return str;
      } // 日期对象
      ...
      str = $.trim(str);
      var re = REG.exec(str); // 分析结果，将会成为一个数组比如 [2015,9,24,12,23,11]
      // 数组元素至少要到 3 个才有效
     ...
     */
    parseTime: function parseTime(str, regex) {
      // 本身就是时间对象
      ...
      _t.key = _t.key_min + ":" + _t.ss; // 自动显示

      _t.T = _t.H + (_t.m == 0 ? "" : ":" + _t.m);
      _t.TT = _t.HH + (_t.m == 0 ? "" : ":" + _t.mm); // 12小时制支持
      ...
      _t.xm = _t.H == _t.H12 ? "a" : "p";
      _t.PM = _t.H == _t.H12 ? "" : "P";
      _t.pm = _t.H == _t.H12 ? "" : "p"; // 12小时制自动显示

      _t.T12 = _t.H12 + (_t.m == 0 ? "" : ":" + _t.m);
      _t.TT12 = _t.HH12 + (_t.m == 0 ? "" : ":" + _t.mm); // 返回

      return _t;
    },
    //.............................................
    // 解析时间字符串（替代 parseTime)
    parseTimeInfo: function parseTimeInfo(input, dft) {
      if (_.isNull(input) || _.isUndefined(input)) return null; // 接受日期对象
      ...
      var _pad = function _pad(v, width) {
        width = width || 2;
      ...
      var inType = _typeof(input);

      var ms = 0;
      var ti = {}; // 字符串

      if ("string" == inType) {
        var m = /^([0-9]{1,2}):([0-9]{1,2})(:([0-9]{1,2})([.,]([0-9]{1,3}))?)?$/.exec(input);
        if (!m) throw "Not a Time: '" + input + "'!!"; // 仅仅到分钟
      ...
      ti.valueInMillisecond = ti.value * 1000 + ti.millisecond; // 增加一个函数
    ...
    //  12Hm - 12小时制，精确到分钟
    //  12Hs - 12小时制，精确到秒
    timeText: function timeText(t, mode) {
      var str = mode || "24H";
    ...
    返回数组:
        ...
        - false : 闭
    */
    region: function region(str, formatFunc) {
      // 处理格式化值的方式
      if (formatFunc) {
        var fft = _typeof(formatFunc);

        if ("string" == fft) {
          // 日期
          if ("date" == formatFunc) {
            formatFunc = function formatFunc(v) {
              return AsDate(v);
            };
          } // 时间
      ...
      var s = str.replace(/[ \t]/g, ""); // eval:  |   1  ||  2  || 3 || 4  ||  5   |
      ...
      re.match = function (v) {
        ...
        if (this.length == 4) {
          if (null != this[1]) {
            if (this[0] && this[1] >= v || !this[0] && this[1] > v) return false;
          }

          if (null != this[2]) {
            if (this[3] && this[2] <= v || !this[3] && this[2] < v) return false;
          }
        ...
        if (this[0] && this[2]) return this[1] != v; // 等于
    ...
    updateColor: function updateColor(color) {
      if (!_.isNumber(color.alpha)) color.alpha = 1;
     ...
     */
    parseColor: function parseColor(str, alpha) {
      // 初始颜色是黑色
      var color = {
        red: 0,
        green: 0,
      ...
      if (!str) return this.updateColor(color); // 本来就是一个颜色对象，创建个新的
    ...
    setInputVal: function setInputVal(jInput, val) {
      var dft = jInput.attr("placeholder");
      if (dft == val) jInput.val("");else jInput.val(val);
    },
    //.............................................
    // 获得页面的锚值，即在 href 后面的
    pageAnchor: function pageAnchor() {
      ...
      if (pos > 0) return href.substring(pos + 1);
      return null;
    },
    //.............................................
    ...
    */
    parseHref: function parseHref(href, decode) {
      href = $.trim(href);
      if (!href) return null; // 首先按照 javascript 来理解
      ...
      if (m) {
        ...
        if (qs) {
          ...
          for (var i = 0; i < pp.length; i++) {
            ...
            if (mp) {
              var pv = mp[3] || "";
              if (decode) pv = decodeURIComponent(pv);
              params[mp[1]] = pv;
            }
          }
        } // 得到数据
    ...
    // @return 一个 URI 字符串
    renderHref: function renderHref(ho, encode) {
      // 返回拼合的字符串
      var re = [];
      if (ho.href) re.push(ho.href); // 参数
      ...
      if (ho.anchor) {
        if (/^#/.test(ho.anchor)) re.push(ho.anchor);else re.push("#" + ho.anchor);
      }

      return encode ? encodeURI(re.join("")) : re.join("");
    },
    //.............................................
    // 得到函数体的代码
    // 因为这种方法通常用来获得内嵌 HTML 文本，
    // 参数 removeComment 如果声明了，将会移除文本前后的 '/*' 和 '*/'
    getFuncBodyAsStr: function getFuncBodyAsStr(func, removeComment) {
      var str = func.toString();
      var posL = str.indexOf("{");
      var re = $.trim(str.substring(posL + 1, str.length - 1)); // Safari 会自己加一个语句结尾，靠

      if (re[re.length - 1] == ";") re = re.substring(0, re.length - 1); // 移除前后的注释

      if (removeComment) re = re.substring(2, re.length - 2); // 嗯，搞好了
    ...
    @return 对象数组
    */
    parseLine: function parseLine(str, sep, keys, format) {
      str = $.trim(str); // 空字符串

      if (!str) return []; // 准备行解析函数

      var _do_line; // 字符串分隔


      if (_.isString(sep)) {
        _do_line = function _do_line(line, sep, keys) {
          ...
          _do_line = function _do_line(line, sep, keys) {
            ...
            _do_line = function _do_line(line, sep, keys) {
              return _.isArray(keys) && keys.length > 0 ? zUtil.obj(keys[0], line) : line;
            };
          } // 准备返回值
    ...
    concat: function concat() {
      if (arguments.length > 0) {
        ...
        if (!re) return [];
        if (!_.isArray(re)) re = [re];

        for (i++; i < arguments.length; i++) {
          var arg = arguments[i];
          if (arg) re = re.concat(arg);
        }
    ...
    setUndefined: function setUndefined(obj, key) {
      // 后面的的默认值挨个试
      if (_.isUndefined(obj[key])) for (var i = 2; i < arguments.length; i++) {
        var val = arguments[i];
    ...
    fallbackUndefined: function fallbackUndefined() {
    ...
    fallback: function fallback(pass, args, dft) {
      for (var i = 0; i < args.length; i++) {
        var val = args[i]; // 看看是否 pass
    ...
    //.............................................
    jq: function jq(jP, arg, selector) {
      ...
      if (_.isNumber(arg)) {
        if (selector) return jP.find(selector).eq(arg);
    ...
    eachTextNode: function eachTextNode(jq, callback) {
      jq = $(jq);
    ...
    escapeText: function escapeText(str, trim) {
      return _.isString(str) ? str.replace("<", "&lt;") : str; //re = this.__escape_ele.text(str).text();
    },
    // 对 HTML 代码逃逸
    escapeHtml: function escapeHtml(str, trim) {
      ...
      while (m) {
        var ms = m[0];
        var bg = m.index;
        var ed = m.index + ms.length; // 补充前面的
    ...
    // 对 HTML 代码逃逸的结果，反逃逸
    unescapeHtml: function unescapeHtml(str, trim) {
      // var re = _.isString(str) ? str.replace("<", "&lt;") : str;
      //re = this.__escape_ele.text(str).text();
      if (!str || !_.isString(str)) return str;
      var re = "";
    ...
    // SVG 相关
    svg: {
      createRoot: function createRoot(styles) {
      ...
      create: function create(type, attrs, styles) {
    ...
    // 函数的返回
    invoke: function invoke(obj, funcName, args, me) {
      if (obj) {
    ...
    declare: function declare(deps, callback) {
    ...
    //.............................................
    // 打开一个新的窗口
    openUrl: function openUrl(url, target, method, params) {
      // 如果默认是 GET 方法
      ...
      if (params) for (var key in params) {
        $('<input type="hidden">').appendTo(jq).prop("name", key).val(params[key]);
      }
      jq[0].submit();
      jq.remove();
    },
    ...
    // 返回列表中随机一个对象
    randomListObj: function randomListObj(list) {
      if (_.isArray(list)) {
        if (list.length > 0) {
    ...
    // 用这个函数应该别 extend 要快点
    // 函数返回 obj 本身
    inherit: function inherit(obj, parent) {
    ...
    clone: function clone(obj) {
      ...
      if (_.isNull(obj) || _.isUndefined(obj) || _.isNumber(obj) || _.isBoolean(obj) || _.isString(obj) || _.isFunction(obj)) {
        return obj;
      } // 数组
    ...
    isPlainObj: function isPlainObj(obj) {
      ...
      if (this.isjQuery(obj)) return false;
    ...
    isEndsWith: function isEndsWith(str, sub) {
      if (!str || !sub) return false;
      if (sub.length > str.length) return false;
      return str.substring(str.length - sub.length) == sub;
    },
    isStartsWith: function isStartsWith(str, sub) {
      if (!str || !sub) return false;
      if (sub.length > str.length) return false;
    ...
    compactHTML: function compactHTML(html, msgMap, settings) {
      html = (html || "").replace(/[ ]*\r?\n[ ]*/g, "");
    ...
    //.............................................
    // 将属性设置到控件的 DOM 上
    setJsonToSubScriptEle: function setJsonToSubScriptEle(jq, className, prop, needFormatJson) {
      ...
      var RP = function RP(key, val) {
        if (/^__/.test(key)) return undefined;
        return val; //return val;
      };

      var json = needFormatJson ? "\n" + $z.toJson(prop, RP, '    ') + "\n" : $z.toJson(prop, RP); // 逃逸一下 HTML

      json = this.escapeHtml(json); // 存储

      jPropEle.text(json); //console.log(jPropEle.text())
    },
    // 从控件的 DOM 上获取控件的属性
    getJsonFromSubScriptEle: function getJsonFromSubScriptEle(jq, className, dft) {
      var jPropEle = jq.children("script." + className);

      if (jPropEle.length > 0) {
        var json = jPropEle.text(); // 反逃逸 HTML
     ...
     */
    getRelativePath: function getRelativePath(base, path, equalPath) {
      if (base == path) {
        return equalPath;
      } // 开始判断
      ...
      var dir = 1;
      if (/\/$/.test(base)) dir = 0; //console.log("the dir=", dir)

      var sb = zUtil.dupString("../", bb.length - pos - dir);
      var pss = ff.splice(pos);
      var rph = sb + pss.join("/"); //console.log(base, path, "=", rph);
     ...
     */
    //.............................................
    getParentPath: function getParentPath(ph, regular) {
     ...
     * @return 整理后的路径
     */
    getCanonicalPath: function getCanonicalPath(path) {
      if (!path) return path;
     ...
     */
    isInPath: function isInPath(base, path) {
      if (path.length <= base.length) return false; // 截取前面一段，必须完全相等
      ...
      if (base != path.substring(0, len)) return false; // 剩下的部分， path 必须以 '/' 开头
     ...
     */
    removeIt: function removeIt(jq, opt) {
      // 格式化参数
      jq = $(jq);
      opt = opt || {};
      ...
      var h = jq.outerHeight(true); // 得到被删除目标的真实样式

      var eleStyle = window.getComputedStyle(jq[0]); // 增加占位对象，以及移动 me
     ...
     */
    editIt: function editIt(ele, opt) {
      // 处理参数
      var jEle = $(ele);
      if (jEle.size() == 0 || jEle.attr("z-edit-it-on")) return; // 标记已经被编辑
      ...
      var __on_input = function __on_input(e) {
        var jInput = $(this);
        var jDiv = jInput.parent();
        var opt = jDiv.data("@OPT"); // 如果是自动延伸 ...
      ...
      var __on_keydown = function __on_keydown(e) {
        e.stopPropagation();
        var jInput = $(this);
        var jDiv = jInput.parent();
        var opt = jDiv.data("@OPT"); //console.log(e.which)
        // Esc
        ...
        else if (46 == e.which) {} //e.stopPropagation();
          // Enter
          else if (13 == e.which) {
              // 多行的话，必须加 ctrl 才算确认
              if (opt.multi) {
      ...
      var __on_ok = function __on_ok() {
        var jInput = $(this);
        var jDiv = jInput.parent();
        var jEle = jDiv.data("@ELE");
        var opt = jDiv.data("@OPT");
        var context = opt.context || jEle; // 处理值

        var old = jDiv.data("@OLD");
        var val = jInput.val(); // 单行的默认要 trim 一下
        ...
        if (opt.multi && opt.newLineAsBr) {
          val = val.replace("<", "&lt;").replace(">", "&gt;").replace(/\r?\n/g, "\n<br>");
        } // 回来吧宿主
        ...
        jEle.removeAttr("z-edit-it-on"); // 调用回调
      ...
      var jDiv = $(html).data("@OPT", opt).data("@ELE", jEle).data("@OLD", val).attr({
        "org-width": boxW,
        "org-height": boxH
      }).css({
        "width": boxW,
      ...
      if (!opt.multi) {
        ...
        if (opt.selectOnFocus) jInput[0].select();
      } //...............................................
      // 取得宿主的显示模式
      ...
      if (opt.copyStyle) {
        ...
        var rKeys = ["display", "letter-spacing", "margin", "padding", "font-size", "font-family", "border", "line-height", "text-align"]; // 如果占位模式，才 copy 背景色和前景色
        ...
        for (var i = 0; i < rKeys.length; i++) {
          ...
          css[rKey] = eleStyle[pKey]; //console.log(rKey, " : ", eleStyle[pKey]);
        } //console.log(css);
        // 将自身设置成和宿主一样的显示模式
      ...
      if (opt.takePlace) {
        // 占宿主的位置
        jDiv.insertBefore(jEle); // 然后，嗯，宿主死开

        jEle.appendTo(jEle[0].ownerDocument.body).hide(); // 模拟宿主点击
      ...
      jInput.on("keydown", __on_keydown); // jInput.on("click", function(e){
      //     console.log(e.isPropagationStopped());
      // })

      jInput.focus(); // 返回最新的 DIV
     ...
     */
    markIt: function markIt(opt, callback) {
      // 支持快捷方式 ({..}, callback)
      ...
      if (!_.isArray(opt.items) || opt.items.length == 0) return; // 支持设置回调

      this.setUndefined(opt, "done", callback); // 绘制遮罩层
      ...
      var R_VP = $D.rect.gen(jMark); //console.log(R_VP)

      var jCanv = $('<canvas>').appendTo(jMark).attr({
        "width": R_VP.width,
        "height": R_VP.height
      });
      var canvas = jCanv[0]; // 准备文字层
      ...
      var __draw_item = function __draw_item(it) {
        if (!it.target) return;
        var jTa = $(it.target);
        if (jTa.length == 0) return; // 得到画笔，并绘制初始背景色
        ...
        g2d.fillRect(0, 0, canvas.width, canvas.height); // 重置画笔

        g2d.fillStyle = "#F80";
        g2d.strokeStyle = "#F80";
        g2d.lineWidth = 2; // 计算矩形

        var rect = $D.rect.gen(jTa);
        console.log(rect);
        var args = zUtil.pick(rect, "left,top,width,height", true); // 绘制提示区域高亮矩形

        g2d.clearRect.apply(g2d, args);
        g2d.strokeRect.apply(g2d, args); // 绘制文字
        // 看看左右两个距离哪个大
      ...
      __draw_item(opt.items[index++]); //  监控鼠标
    ...
    */
    tabArticle: function tabArticle(jAr, isTab) {
      if (!jAr || jAr.length == 0) return;
      var is_first_level = false; // 标记过的，就不要标记了

      if (jAr.attr("z-article-tab-already")) return; // 默认用 H1 判断

      if (!isTab) {
        is_first_level = true;

        isTab = function isTab(jq, index) {
          if (index >= jq.length - 1) return null;
          var ele = jq[index];
          var el2 = jq[index + 1];
      ...
      for (var i = 0; i < jq.length; i++) {
        var ele = jq[i];
        var blo = isTab(jq, i); // 当前是 Hr 且前面是一个 H1，准备收缩的块
      ...
      if (blist.length > 0) {
        ...
        var jTabs = $('<div class="z-article-tabs">').insertBefore(jStub);
        ...
        for (var i = 0; i < blist.length; i++) {
          ...
          if (b.children.length > 0) {
            ...
            jBlock.append($(b.children)); // 针对每个块进行第二级缩进
    ...
    //.............................................
    untabArticle: function untabArticle(jAr) {
      if (!jAr || jAr.length == 0) return; // 未标识过，无需释放

      if (!jAr.attr("z-article-tab-already")) return; // 首先移除所有标签

      jAr.find('.z-article-tabs').remove(); // 循环块

      jAr.find('>.z-article-block').each(function () {
        var jBlock = $(this); // 解开二级包裹标签
    ...
    tabArticleMarkCurrent: function tabArticleMarkCurrent(jAr, selector) {
      if (!jAr || jAr.length == 0) return;
      var bIndex = 0;
      var bArea = 0;
      var viewport = $D.dom.winsz(jAr[0].ownerDocument.defaultView); //console.log("haha")
    ...
    // 处理页面自动停靠相关的功能
    pageDock: {
      unmark: function unmark(ele) {
        // 确保是个 jQuery
        var jq = $(ele); // 删除属性和标识

        jq.removeData("z-dock-info").removeAttr("z-dock-ele").css({
          "position": "",
          "top": "",
          "left": "",
          "right": "",
      ...
      */
      mark: function mark(ele, opt) {
        ...
        $z.setUndefined(opt, "reset", false); // 没内容无视

        if (ele.length == 0) return; // 多个元素，逐个标记
        ...
        var list = doc.__dock_list_top; // 查找一下
      ...
      */
      adjust: function adjust(doc) {
        ...
        for (var i = 0; i < dockList.length; i++) {
          var jq = $(dockList[i]);
          var dt = jq.data("z-dock-info");
          var threshold = dt.primerTop + dt.primerHeight * dt.factor - topH; //console.log("scroll",index, docScrollTop, threshold);
          ...
          if (docScrollTop > threshold) {
            ...
            if (dt.detectParent) {
              ...
              if (jMyPaOut.length == 0) {
                ...
                var ovRect = $D.rect.overlap(viewport, inRect); // 自己所在块与视口重叠的面积必须要能放置自身的高度
          ...
          if (is_should_dock) {
            ...
            jq.css(css).attr("z-dock-enabled", "yes"); // 累计下一个堆叠起点
    ...
    // },
    //.............................................
    toJson: function toJson(obj, fltFunc, tab) {
      return JSON.stringify(obj, fltFunc, tab);
    },
    //.............................................
    fromJson: function fromJson(str, fltFunc) {
      str = $.trim(str);
      if (!str) return null;
    ...
    arrayToMap: function arrayToMap(ary, key) {
      var map = {};
    ...
    countStrHeadIndent: function countStrHeadIndent(str, tabWidth) {
      var n = 0;
      tabWidth = tabWidth || 4;
      ...
      if (str) {
        for (var i = 0; i < str.length; i++) {
          var c = str.charAt(i);
          if (' ' == c) n++;else if ('\t' == c) n += tabWidth;else break;
        }
      }
    ...
    // str : 给定字符串
    // c : 指定重复字符
    countStrHeadChar: function countStrHeadChar(str, c) {
      var re = 0;
      if (str) for (; re < str.length; re++) {
        if (str.charAt(re) != c) return re;
      }
      return re;
    },
    //............................................
    ...
    //  - sub : 要计算重复的子字符串
    // 返回出现的次数
    countSubStr: function countSubStr(str, sub) {
      var re = 0;
    ...
    // tabWidth : 一个 \t 相当于几个空格，默认 4
    shiftIndent: function shiftIndent(str, indent, tabWidth) {
      if (!str) return str;
      ...
      for (; i < str.length; i++) {
        if (n > 0 && Math.floor(n / tabWidth) >= indent) break;
        var c = str.charAt(i);
        if (' ' == c) n++;else if ('\t' == c) n += tabWidth;else break;
      }

      if (i > 0) return str.substring(i);
      return str;
    },
     ...
     */
    markdownToHtml: function markdownToHtml(str, opt) {
      // 确保有选项对象
      opt = opt || {};
      context = opt.context || this; // 设置默认值
      ...
      for (var i = 0; i < lines.length; i++) {
        var line = lines[i];
        var trim = $.trim(line);
        var indent = zUtil.countStrHeadIndent(line, 2); // 来吧，判断类型
        // 空段落
        ...
        if (trim.length == 0) {
          ...
          if (/^(code|UL|OL)$/.test(B.type)) {
            ...
            B.content.push($.trim(line.substring(B.level)));
              ...
              B = blocks.tryPush(B);
                ...
                B = blocks.tryPush(B);
              ...
              else if ("P" == B.type && B.content.length == 1 && B.content[0].indexOf("|") > 0 && /^[ |:-]{6,}$/.test(line)) {
                  // 修改之前段落的属性
                  B.type = "Th";
                  B.content = zUtil.splitIgnoreEmpty(B.content[0], "|"); // 解析自己，分析单元格的 align
                  ...
                  B = blocks.tryPush(B); // 标识后续类型为 Tr

                  B.type = "Tr";
                } // 有序列表: OL
                else if ((!B.type || /^(OL|UL)$/.test(B.type)) && /^[0-9a-z][.]/.test(trim)) {
                    B = blocks.tryPush(B);
                    B.type = "OL";
                    B.level = indent;
                    B.content.push($.trim(trim.substring(trim.indexOf('.') + 1)));
                  } // 无序列表: UL
                  else if ((!B.type || /^(OL|UL)$/.test(B.type)) && /^[*+-][ ]/.test(trim)) {
                      B = blocks.tryPush(B);
                      B.type = "UL";
                      B.level = indent;
                      B.content.push($.trim(trim.substring(1)));
                        ...
                        else if (/^(OL|UL|quote|P)$/.test(B.type) && (!lastLineIsBlankLink || indent > B.level)) {
                            B.content.push(trim);
                          } // 将自己作为表格行
                          else if ("Tr" == B.type) {
      ...
      B = blocks.tryPush(B); //console.log(blocks)
      // 定义内容输出函数

      var __B_to_html = function __B_to_html(tagName, B) {
        // 开始渲染
        var html = "";
        var tagNames = {};

        for (var i = 0; i < B.content.length; i++) {
          var line = B.content[i]; // 忽略空行

          if (!line) continue; // 首行以后每行前都加个换行符
      ...
      var __line_to_html = function __line_to_html(str, tagNames) {
        var reg = '(\\*([^*]+)\\*)' + '|(\\*\\*([^*]+)\\*\\*)' + '|(__([^_]+)__)' + '|(~~([^~]+)~~)' + '|(`([^`]+)`)' + '|(!\\[([^\\]]*)\\]\\(([^\\)]+)\\))' + '|(\\[(' + '(!\\[([^\\]]*)\\]\\(([^\\)]+)\\))|([^\\]]*)' + ')\\]\\(([^\\)]*)\\))' + '|(https?:\\/\\/[^ ]+)';
        ...
        while (m = REG.exec(str)) {
          ...
          if (m[1]) {
            html += '<em>' + __line_to_html(m[2]) + '</em>'; // 记录标签

            if (tagNames) tagNames["em"] = true;
          } // B: **xxx**
          else if (m[3]) {
              html += '<b>' + __line_to_html(m[4]) + '</b>'; // 记录标签

              if (tagNames) tagNames["b"] = true;
            } // U: __xxx__
            else if (m[5]) {
                html += '<u>' + __line_to_html(m[6]) + '</u>'; // 记录标签

                if (tagNames) tagNames["b"] = true;
              } // DEL: ~~xxx~~
              else if (m[7]) {
                  html += '<del>' + __line_to_html(m[8]) + '</del>'; // 记录标签

                  if (tagNames) tagNames["del"] = true;
                } // CODE: `xxx`
                else if (m[9]) {
                    var s2 = m[10]; // 特殊文字
                    ...
                    if (tagNames) tagNames["code"] = true;
                  } // IMG: ![](xxxx)
                  else if (m[11]) {
                      //console.log("haha", m[13])
                      var src = m[13];
                      var src2 = opt.media.apply(context, [src]); // 处理一下 alt
                      ...
                      if (iH) cssImg.push("height:" + iH + (/[0-9]$/.test(iW) ? "px;" : ";")); // 判断是否是视频

                      var isVideo = /[.](mp4|avi|mov)$/.test(src); // 准备输出头

                      html += isVideo ? '<video controls ' : '<img '; // 源

                      html += ' src="' + src2 + '" '; // 宽高

                      if (cssImg.length > 0) html += ' style="' + cssImg.join("") + '"'; // 提示信息

                      if (alt) html += ' title="' + alt + '"'; // 结束
                      ...
                      if (tagNames) tagNames["img"] = true;
                        ...
                        if (m[16]) {
                          ...
                          if (tagNames) tagNames["img"] = true;
                            ...
                            var text = m[19] || zUtil.getMajorName(href || ""); //console.log("A", href, text)
                            // 锚点
                            ...
                            if (!href && /^#.+$/.test(text)) {
                              html += '<a name="' + text.substring(1) + '"></a>';
                                ...
                                html += '<a ' + (newtab ? 'target="_blank' : "") + ' href="' + href + '">' + __line_to_html(text) + '</a>';
                              }
                          } // 记录标签


                        if (tagNames) tagNames["a"] = true;
                      } // A: http://xxxx
                      else if (m[21]) {
                          html += '<a href="' + m[21] + '">' + m[21] + '</a>'; // 记录标签
      ...
      var __B_to_blockquote = function __B_to_blockquote(B, c) {
        c.html += __B_to_html("blockquote", B); // 循环查找后续的嵌套块
      ...
      var __B_test_task = function __B_test_task(B) {
      ...
      var __B_to_list = function __B_to_list(B, c) {
        __B_test_task(B); // 开始渲染
        ...
        c.html += '>';
        c.html += __B_to_html("li", B); // 循环查找后续的列表项，或者是嵌套

        for (c.index++; c.index < c.blocks.length; c.index++) {
          var B2 = c.blocks[c.index]; // 继续增加
            ...
            re.html += '</pre>';
              ...
              __B_to_list(B, re);
                ...
                re.html += '\n<hr>';
                  ...
                  var THead = B;
                  var aligns = THead.cellAligns || []; // 输出表头

                  re.html += '\n<thead>\n<tr>';

                  for (var x = 0; x < B.content.length; x++) {
                    var align = "left";
                    if (x < aligns.length) align = aligns[x];
    ...
    */
    parsePoster: function parsePoster(str, opt) {
      str = str || "";
      opt = opt || {}; // 设置默认值
      ...
      var C = opt.context || this; //console.log(C)
      // 准备解析结果
      ...
      var __join_poster_item = function __join_poster_item(it, poItem, m_attr, m_css) {
        // 必须有 poItem
        if (!poItem) {
        ...
        poItem.attr = m_attr ? zUtil.fromJson(m_attr) : null; // 如果 it 是组，且给定项目的 depth 深
        ...
        if (it && 'group' == it.type && poItem.depth > 0) {
          ...
          grp.items.push(poItem); // 继续返回自己

          return it;
        } // 加入到全局


        poster.items.push(poItem); // 如果是组，那么就返回
      ...
      for (var i = 0; i < lines.length; i++) {
        var line = lines[i];
        var trim = $.trim(line); //console.log(line)
        // 计算缩进级别

        var depth = zUtil.countStrHeadIndent(line, 2); // 忽略注释和空行

        if (!trim || /^#/.test(trim)) continue; //....................................
        // 全局属性
        ...
        if (m) {
          ...
          var m_ta = m[1];
          var m_tp = m[2];
          var m_str = m[4]; // 如果是 >bg 或者 >bgcolor ...

          var key = m_tp;
          if ('%' == m_ta) key = "con_" + m_tp;
        ...
        if (!m) continue; // 分析正则表达式

        var m_tp = m[1];
        var m_sel = m[3];
        var m_attr = m[4];
        var m_css = m[6];
        var m_str = m[8]; // 根据类型判断
        ...
        if (/^(text|spec|list)$/.test(poItem.type)) {
          ...
          if (m_str) ss.push(m_str); // 读取内容，一直读到结束标记

          for (i++; i < lines.length; i++) {
            line = $.trim(lines[i]); // 遇到组，或者其他元素结束

            if (/^((- *group)|(\+ *(text|picture|video|spec|list)))/.test(line)) break;
            ss.push(line);
          } // 不要忘记退一格
    ...
    // @return jDiv
    renderPoster: function renderPoster(poster, jDiv) {
      jDiv = jDiv || $("<div>"); //----------------------------
      // 样式

      if (poster.cssText) jDiv[0].style.cssText = poster.cssText; // 全局属性

      if (poster.attr) jDiv.attr(poster.attr); //----------------------------
      // 宽度属性
      ...
      if (poster.layout) jDiv.attr("poster-layout", poster.layout); //----------------------------
      // 优先 CSS

      var css = {}; // 背景图

      if (poster.bg) css.backgroundImage = 'url("' + poster.bg + '")'; // 背景颜色

      if (poster.bgcolor) css.backgroundColor = poster.bgcolor; // 前景颜色

      if (poster.color) css.color = poster.color; // 设置 CSS
      ...
      if (poster.con_bg) css.backgroundImage = 'url("' + poster.con_bg + '")'; // 背景颜色

      if (poster.con_bgcolor) css.backgroundColor = poster.con_bgcolor; // 设置 CSS

      jInner.css(css); //----------------------------
      // 声明处理函数

      var __do_item = function __do_item(jP, it) {
        var jIt; // 组
            ...
            jIt = $('<span>').html(txt.replace(/\r?\n/g, '<br>') || "&nbsp;");
              ...
              jIt = $('<img>').attr("src", it.picture);
                ...
                jIt = $('<video>').attr("src", it.video);
                  ...
                  jIt = $('<div it="spec">');
                  var jT = $('<div class="spec-body">').appendTo(jIt); // 表格内容

                  if (_.isArray(it.spec.rows)) for (var y = 0; y < it.spec.rows.length; y++) {
                    ...
                    for (var i = 0; i < it.list.length; i++) {
                      ...
                      if (li.src) {
                        $('<img>').attr("src", li.src).appendTo(jLiCon);
                      }

                      if (li.text) {
                        $('<span>').html(li.text.replace(/\r?\n/g, '<br>')).appendTo(jLiCon);
                      }
                    }
                  } // 不是合法的项目，无视


        if (!jIt) return; // 增加类选择器

        if (it.selector) jIt.addClass(it.selector); // 增加样式

        if (it.cssText) jIt[0].style.cssText = it.cssText; // 增加属性

        if (it.attr) jIt.attr(it.attr); // 加入 DOM
    ...
    */
    explainPoster: function explainPoster(jq, opt) {
      // 格式化输入
      jq = $(jq);
      opt = opt || {};
      opt.className = opt.className || "md-code-poster";
      if (jq.length == 0) return; // 循环处理
    ...
    */
    parseLayoutSize: function parseLayoutSize(str, dft) {
      s = $.trim(str);

      if (!s) {
        if (_.isUndefined(dft)) return {};
        return dft;
      } // 准备解析函数


      var ___layout_size = function ___layout_size(sz) {
        if (_.isNumber(sz)) return sz + "px"; // 确保去掉空白

        sz = $.trim(sz);
        if ("?" == sz || !sz) return undefined;
        if ("-" == sz) return "hidden";
        if (/^[0-9]+$/.test(sz)) return parseInt(sz);
        return sz;
      };
    ...
    */
    wrapVideoSimplePlayCtrl: function wrapVideoSimplePlayCtrl(jVideo, opt) {
      jVideo = $(jVideo); // 防空

      if (!jVideo || jVideo.length == 0) return; // 已经设置过了

      if (jVideo.parent().hasClass(".z-video-con")) return; // 多个视频
      ...
      var userAgent = window.navigator.userAgent; // alert(userAgent);
      ...
      this.setUndefined(opt, "copyAttrs", ["pos", "itfree", "ithide", "itfull", "itshadow"]); // 创建包裹

      var jCon = $('<div class="z-video-con">').insertBefore(jVideo);
      jCon.append(jVideo);
      if (opt.autoFitHeight) jCon.css("height", "100%");
    ...
    //.............................................
    // 返回当前时间
    currentTime: function currentTime(date) {
    ...
    // 返回当前时分秒
    dateToYYMMDD: function dateToYYMMDD(date, split) {
    ...
    // 返回当前年月日
    dateToHHMMSS: function dateToHHMMSS(date, split) {
      date = date ? this.parseDate(date) : new Date();
      split = split == null || split == undefined ? "-" : split;
    ...
    // 任何东西转换为字符串
    anyToString: function anyToString(obj) {
    ...
    // 补全右边
    alignLeft: function alignLeft(str, length, _char) {
      ...
      return str + zUtil.dupString(_char || ' ', length - str.length);
    },
    // 补全左边
    alignRight: function alignRight(str, length, _char2) {
      str = zUtil.anyToString(str);
      ...
      return zUtil.dupString(_char2 || ' ', length - str.length) + str;
    },
    // 重复字符
    dupString: function dupString(_char3, num) {
      if (!_char3 || num < 1) {
        return "";
      }
     ...
     * @return 转换后字符串
     */
    lowerWord: function lowerWord(cs, c) {
      var c = c || "-";
      var sb = "";
     ...
     * @return 转换后字符串
     */
    upperWord: function upperWord(cs) {
      var c = c || "-";
      var sb = "";
      var len = cs.length;
    ...
    upperFirst: function upperFirst(s) {
      if (!s) return s;
      var c = s.charAt(0);
      if (/[a-z]/.test(c)) return c.toUpperCase() + s.substring(1);
      return s;
    },
    ...
    lowerFirst: function lowerFirst(s) {
      if (!s) return s;
      var c = s.charAt(0);
      if (/[A-Z]/.test(c)) return c.toLowerCase() + s.substring(1);
      return s;
    },
    // 显示一个元素的尺寸，调试用
    _dumpSize: function _dumpSize(ele) {
      var jq = $(ele);
    ...
    // 未实现
    noImplement: function noImplement() {
      throw new Error("Not implement yet!");
    },
    // 将字符串拆分，并无视空字符串
    splitIgnoreEmpty: function splitIgnoreEmpty(str, separator) {
    ...
    // 将字符串拆分，并无视空字符串
    splitIgnoreBlank: function splitIgnoreBlank(str, separator) {
      var ss = str.split(separator || ",");
      var re = [];
    ...
    //============== 计算文件大小
    sizeText: function sizeText(sz, fix) {
      sz = parseInt(sz) || 0;
      fix = fix || 2; // KB
    ...
    // 125: [}]
    // 126: [~]
    evalPassword: function evalPassword(str, minLen) {
      // 空密码
      if (!str) return 0; // 密码长度不足
      ...
      for (var i = 0; i < str.length; i++) {
        var c = str.charCodeAt(i); // 非法

        if (c < 33 || c > 126) return -2; // 数字
    ...
    ellipsisCenter: function ellipsisCenter(str, len) {
    ...
    formatJson: function formatJson(obj, depth) {
      var type = _typeof(obj); // 空对象


      if (null == obj && ("object" == type || 'undefined' == type || "unknown" == type)) return 'null'; // 字符串
      ...
      if ("boolean" == type) return obj ? "true" : "false"; // 数字

      if ("number" == type) return obj; // 是否需要格式化
      ...
      if ('function' == type) return '"function(){...}"'; // 普通 JS 对象
  ...
  window.$z = zUtil; // TODO 支持 AMD | CMD
  //===============================================================
  // if (typeof define === "function") {
  //     // CMD
  //     if (define.cmd) {
