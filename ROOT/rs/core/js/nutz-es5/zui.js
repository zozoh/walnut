...
define(function (require, exports, module) {
  ...
  var parse_dom = function parse_dom(html) {
    var UI = this; // 解析代码模板
    // html = html.replace(/[ ]*\r?\n[ ]*/g, "");
    // html = $z.tmpl(html)(UI._msg_map);

    html = UI.compactHTML(html); // 找到要加入 HTML 的位置并替换 HTML

    var jCon = UI.findDomParent();

    if (jCon && jCon.length > 0) {
      jCon.html(html); // 展开内嵌的 DOM 节点
    ...
    if (jTmpl) {
      ...
      jTmpl.children('[code-id]').each(function () {
        var jq = $(this);
        if (commonClass && jq.attr("nocommon") != "true") jq.addClass(commonClass);
    ...
    sign_gaskets(UI); // DOM 解析完成
  };

  var sign_gaskets = function sign_gaskets(UI) {
    // 搜索所有的 DOM 扩展点, 为其增加前缀
    UI.$el.find("[ui-gasket]").each(function () {
      var jq = $(this);
      var ga = jq.attr("ui-gasket"); // 已经被标识过了的话
  ...
  var register = function register(UI) {
    var opt = UI.options; //....................................
    // 直接指明了 $el，那么需要进行替换
    ...
    if (UI.$el) {
      ...
      UI.$pel = UI.$el.parent();
      UI.pel = UI.$pel[0];
      UI.parent = UI.parent || ZUI.getInstance(UI.pel); // 这种模式下，默认是要 keepDom 的

      $z.setUndefined(UI, "keepDom", _.isUndefined(opt.keepDom) || opt.keepDom);
      UI.keepDom = _.isUndefined(opt.keepDom) ? UI.keepDom === false ? false : true : opt.keepDom; // if(UI.keepDom)
      ...
      var cid = UI.$el.attr("ui-id"); // 如果本身就是一个 UI，则试图注销它
    ...
    // 指定了 $pel 的话 ...
        ...
        UI.parent = UI.parent || ZUI.getInstance(UI.$pel); // 如果这个元素带了 gasketName，则采用它

        var gnm = UI.$pel.attr("ui-gasket");
        var m = /^(\w+)@(\w+)$/.exec(gnm);
        opt.gasketName = m ? m[2] : gnm; // 如果配置里没指定父，那么就得手动来一下
      ...
      // 没有 $pel 则需要寻找选区
          ...
          UI.$pel = UI.parent.$el.find(selector); // 从扩展的搜索范围里去寻找
    ...
    if (!UI.$el) {
      ...
      UI.$el.attr("ui-loadding", "yes").css("visibility", "hidden"); // 加入 DOM 树
    ...
    ZUI.instances[UI.cid] = UI; // 记录顶级 UI
  ...
  ZUIObj.prototype = {
    ...
    __init__: function __init__(options) {
      ...
      UI._defer_keys = {};
      UI._defer_callbacks = []; //............................... 保存配置项

      var opt = options || {};
      UI.options = opt; //............................... 必要的内部字段
      ...
      UI._msg_map = UI.parent ? UI.parent._msg_map : ZUI.g_msg_map; // 注册 UI 实例

      register(UI); //............................... 继承父UI的属性执行器

      UI.exec = opt.exec || (UI.parent || {}).exec;
      UI.app = opt.app || (UI.parent || {}).app; // 得到 UI 的事件

      var events = _.extend({}, UI.events, opt.events); // 监听事件
      ...
      if (opt.className) {
        UI.$el.addClass(isOptResetClassName ? opt.className.substring(1) : opt.className);
      } // 处理通用拖拽操作


      if (UI.dragAndDrop) {
        var ddSelector = UI.dragAndDrop;
        if (!_.isString(ddSelector)) ddSelector = "> .ui-arena";
      ...
      UI.$el.on("mouseenter", '[balloon]', function (e) {
        e.stopPropagation();
        var jq = $(this); // 已经显示了，就无视

        if (jq.attr('balloon-is-on') || jq.attr('balloon-is-disabled')) return; // 得到 UI 对象
        // var UI = ZUI(jq);
        // 解析
        ...
        var pos = m ? m[1] : undefined; // 生成浮动元素

        var jBall = $('<div class="ui-balloon-con">').prependTo(jq);
        $('<span>').html(msg).appendTo(jBall); // 停靠

        var di = $z.dock(jq, jBall, pos); //var di = {};
        // 标识
        ...
        $(this).removeAttr("balloon-is-on").find('>.ui-balloon-con').remove();
      }); // 调用更早的自定义初始化功能

      $z.invoke(opt, "on_before_init", [opt], UI); // 调用子类自定义的 init

      $z.invoke(UI.$ui, "init", [opt], UI); // 触发初始化事件
    ...
    // 包含 ui-gasket
    rebuildGaskets: function rebuildGaskets() {
      sign_gaskets(this);
    },
    //............................................
    // 释放全部自己的子
    releaseAllChildren: function releaseAllChildren(forceRemoveDom) {
      var UI = this; // 释放掉自己所有的子
    ...
    appendTo: function appendTo(pui, target) {
      var UI = this; // 首先，将自己从父中去掉

      if (UI.parent != pui) UI.__remove_from_parent(); // 然后加入到 pui 的子列表

      if (pui) pui.__append_child(UI); // 没有指定 target ...

      if (!target) return this; // 直接移动到某指定的元素下面
      ...
      if (_.isElement(target) || $z.isjQuery(target)) {
        ...
        gasketName = m ? m[2] : gnm;
          ...
          UI.$pel = pui.$el.find(selector); // 没找到是不能忍受的哦
      ...
      UI.$el.appendTo(UI.$pel); // 指定了 gasketName
    ...
    //............................................
    __append_child: function __append_child(childUI) {
    ...
    //............................................
    __remove_child: function __remove_child(childUI) {
      var UI = this;
    ...
    //............................................
    __remove_from_parent: function __remove_from_parent() {
      if (this.parent) {
        this.parent.__remove_child(this);
      }
    },
    //............................................
    destroy: function destroy(forceRemoveDom, dontNeedRemoveFromParent) {
      var UI = this;
      var opt = UI.options; //console.log("destroy UI:", UI.uiName, "::", UI.cid);
      // 释放通用拖拽操作
      ...
      UI.trigger("ui:depose", UI); // 释放掉自己所有的子

      UI.releaseAllChildren(forceRemoveDom); // 移除自己在父节点的记录
      ...
      UI.stopListening(); // 移除自己的监听的键盘和鼠标事件

      UI.unwatchKey();
      UI.unwatchMouse(); // 移除 DOM 的事件监听

      $z.invoke(UI.$el, "undelegate", []);
      UI.$el.off(); // 删除自己的 DOM 节点
      ...
      delete ZUI.instances[UI.cid]; // 移除 uiKey
      ...
      ZUI.tops.forEach(function (top) {
        if (top != UI) list.push(top);
      }, UI);
      ZUI.tops = list; // 标识自己的实例，是删除的

      UI.__i_am_dead = true;
    },
    //............................................
    // 渲染自身
    render: function render(afterRender) {
      var UI = this; // 确定语言

      UI.lang = (UI.parent || {}).lang || window.$zui_i18n || "zh-cn";
      UI.theme = window.$zui_theme || "light"; //============================================== i18n读完后的回调
      // 看看是否需要异步加载多国语言字符串
      ...
      var callback = function callback() {
        ...
        var do_after_redraw = function do_after_redraw() {
          // 确保是有效 UI
          if (UI.__i_am_dead) {
            return;
          } // 触发事件
          ...
          UI.trigger("ui:redraw", UI); // 确定 uiKey，并用其索引自身实例
          ...
          if (UI.uiKey) {
            ...
            ZUI._uis[UI.uiKey] = UI; // 看看有没有要延迟监听的
          ...
          UI.$el.removeAttr("ui-loadding").css("visibility", ""); // 触发当前实例的绘制回调
          ...
          if (UI.options.quickTip && UI.options.quickTip.items.length > 0) {
            ...
            UI.__quick_tip = qtip; // 从来没播放过，来一遍
          ...
          UI.trigger("ui:display", UI); // 因为重绘了，看看有木有必要重新计算尺寸，这里用 setTimeout 确保 resize 是最后执行的指令
        ...
        var do_render = function do_render() {
          // 首先看看是否有增加 class
      ...
      var do_i18n = function do_i18n() {
        // 采用父 UI 的字符串
        var uiI18N = $z.concat(UI.$ui.i18n, UI.options.i18n); // console.log(UI.uiName, "UI.$ui.i18n", UI.$ui.i18n);
        // console.log(UI.uiName, "UI.options.i18n", UI.$ui.i18n);
        // 存储多国语言字符串
        // 需要将自己的多国语言字符串与父 UI 的连接

        UI._msg_map = $z.inherit({}, UI.parent ? UI.parent._msg_map : ZUI.g_msg_map); // 找到需要加载的消息字符串

        var i18nLoading = [];

        for (var i = 0; i < uiI18N.length; i++) {
          var it = uiI18N[i]; // 字符串的话，转换后加入待加载列表

          if (_.isString(it)) {
            i18nLoading.push($z.tmpl(it)({
              lang: UI.lang
            }));
          } // 对象的话，直接融合进来
      ...
      var uiCSS = $z.concat(UI.$ui.css, UI.options.css); // 有 CSS 的话，需要对每个项目替换 theme
    ...
    defer: function defer(keys, callback) {
      var UI = this; // 直接添加延迟回调，也就是说，在所有key被 report 以后，再附加一个
      // 行为
    ...
    //............................................
    // 汇报自己完成了哪个延迟加载的 UI
    defer_report: function defer_report(key, quiet) {
      var UI = this; // 因为是延迟，所以放到执行队列最后执行
      ...
      window.setTimeout(function () {
        ...
        UI._defer_keys[key] = false; // 检查加载是否全部完成

        UI.__check_all_defer_done();
      }, 0);
    },
    __check_all_defer_done: function __check_all_defer_done() {
      var UI = this; //console.log("check all done[..]", UI.uiName, $z.toJson(UI._defer_keys));
      // 检查是否所有的 key 都被加载完成了

      for (var key in UI._defer_keys) {
        if (UI._defer_keys[key]) return;
      } //console.log("check all done[OK]", UI.uiName);
      // 进行到这里，表示都完成了，记录一下要调用的函数表


      var callbacks = UI._defer_callbacks; // 清空 defer 的设定

      UI._defer_keys = {};
      UI._defer_callbacks = []; // 依次调用回调
    ...
    getCssTheme: function getCssTheme(cssPath) {
      return $z.tmpl(cssPath)({
        theme: this.theme
      });
    },
    //............................................
    isFitParent: function isFitParent() {
      var UI = this;
      var opt = UI.options;
      return opt.fitparent === true || opt.fitparent !== false && UI.arena.attr("ui-fitparent");
    },
    //............................................
    // 修改 UI 的大小
    resize: function resize(deep) {
      var UI = this;
      var opt = UI.options; // 如果是选择自适应
      ...
      if (UI.arena) {
        ...
        $z.invoke(UI.$ui, "resize", [deep], UI); // 触发事件

        $z.invoke(opt, "on_resize", [], UI);
        UI.trigger("ui:resize", UI); // 调用自身所有的子 UI 的 resize
    ...
    // 除非你在 depose 方法里编写逻辑
    __elements: [],
    addElement: function addElement(jq) {
     ...
     */
    watchKey: function watchKey(which, keys, handler) {
      this.__watch_key_event(ZUI.keymap, which, keys, handler);
    },
    watchKeyUp: function watchKeyUp(which, keys, handler) {
      this.__watch_key_event(ZUI.keyUpmap, which, keys, handler);
    },
    __watch_key_event: function __watch_key_event(kmap, which, keys, handler) {
      // 没有特殊 key
      ...
      var key = this.__keyboard_watch_key(which, keys); //console.log("watchKey : '" + key + "' : " + this.cid);


      $z.pushValue(kmap, [key, this.cid], handler); // 记录到自身以便可以快速索引
    ...
    __keyboard_watch_key: function __keyboard_watch_key(which, keys) {
      // 直接就是字符串
      if (_.isString(which)) {
        return which;
      } // 组合的key
    ...
    // ui.unwatchKey(27)
    unwatchKey: function unwatchKey(which, keys) {
      // 注销全部监听
      ...
      var key = this.__keyboard_watch_key(which, keys); // console.log("unwatchKey : '" + key + "' : " + this.cid);
      // 删除全局索引
      ...
      if (this._watch_keys[key]) delete this._watch_keys[key];
    },
    // 监听全局鼠标事件
    watchMouse: function watchMouse(eventType, handler) {
      $z.pushValue(ZUI.mousemap, [eventType, this.cid], handler);
    },
    // 取消全局鼠标事件监听
    unwatchMouse: function unwatchMouse(eventType) {
      // 去掉所有的事件监听
      if (!eventType) {
        for (var key in ZUI.mousemap) {
          ...
          if (wmm && wmm[this.cid]) delete wmm[this.cid];
        }
    },
    // 对 HTML 去掉空格等多余内容，并进行多国语言替换
    compactHTML: function compactHTML(html, msgMap) {
      return $z.compactHTML(html, msgMap || this._msg_map, this.tmplSettings);
    },
    // 得到多国语言字符串
    msg: function msg(str, ctx, msgMap, dft) {
      if (!str || !_.isString(str)) return str; // 消息字符串是 "e.com.xxx : some reasone" 格式的
    ...
    str: function str(key, dft, ctx) {
      if (/^i18n:.+$/g.test(key)) {
        key = key.substring(5);
      } //var re = $z.getValue(this._msg_map, key);


      return this.msg(key, ctx, null, dft); //return re || dft || "";
    },
    // 对一个字符串进行文本转移，如果是 i18n: 开头采用 i18n
    text: function text(str, ctx, msgMap) {
      // // 多国语言
      // if (/^i18n:.+$/g.test(str)) {
    ...
    // selector 如果不给，默认是 "*"
    balloon: function balloon(selector, enabled) {
      var UI = this;
      return;
      ...
      if (_.isUndefined(enabled)) enabled = true; // 得到选区

      var jq = UI.$el;
      if (_.isString(selector)) jq = jq.find(selector);else if (_.isElement(selector) || $z.isjQuery(selector)) jq = $(selector); // 启用
    ...
    showLoading: function showLoading(msg) {
      ...
      html += '<i class="fa fa-spinner fa-pulse"></i> <span>' + this.msg(msg || "loading") + '</span>';
      html += '</aside></div>';
    ...
    // 释放某个子 UI
    releaseSubUI: function releaseSubUI(uiPath) {
    ...
    local: function local(key, val, appName) {
      var UI = this;
      var app = UI.app;
    ...
    eval_tmpl_func: function eval_tmpl_func(obj, nm) {
      var F = obj ? obj[nm] : null;
      if (!F) return null;
      return _.isFunction(F) ? F : $z.tmpl(F);
    },
    //............................................
    ui_parse_data: function ui_parse_data(obj, callback) {
      var UI = this;
      var opt = UI.options;
      var context = opt.context || UI; // 同步
    ...
    //............................................
    ui_format_data: function ui_format_data(callback) {
      var UI = this;
      var opt = UI.options;
    ...
    //............................................
    // 处理命令的通用回调
    doCommandCallback: function doCommandCallback(re, ok, fail) {
      var UI = this;
      ...
      try {
        var reo;
        if (_.isString(re)) reo = $z.fromJson(re);else reo = re;
    ...
    //............................................
    // 监听本 UI 的模块事件
    listenSelf: function listenSelf(event, handler) {
      this._listen_to(this, event, handler);
    },
    // 监听本 UI 的父UI事件
    listenParent: function listenParent(event, handler) {
      this._listen_to(this.parent, event, handler);
    },
    // 监听某个 UI 的事件
    listenUI: function listenUI(uiKey, event, handler) {
        ...
        this._listen_to(uiKey, event, handler);
          ...
          this._listen_to(this, event, handler);
            ...
            this._listen_to(this.parent, event, handler);
              ...
              var taUI = ZUI.getByKey(uiKey); // 不需要推迟
    ...
    // 监听某个 backbone 的模块消息
    _listen_to: function _listen_to(target, event, handler) {
      if (target) {
    ...
    // 监听父UI的事件
    // 返回一个对应的 DOM 节点的克隆
    ccode: function ccode(codeId) {
      var jq = this._code_templates[codeId];
      if (jq) return jq.clone().removeAttr("code-id");
      throw "Can not found code-template '" + codeId + "'";
    },
    //...................................................................
    ajaxReturn: function ajaxReturn(re, option, context) {
      var UI = this;
      context = context || UI;
    ...
    //...................................................................
    // 得到 UI 的 arena 所占的宽度
    arenaWidth: function arenaWidth(countMargin) {
      if (this.arena) return this.arena.outerWidth(countMargin ? true : false);
    },
    // 得到 UI 的 arena 所占的高度
    arenaHeight: function arenaHeight(countMargin) {
      if (this.arena) return this.arena.outerHeight(countMargin ? true : false);
    },
    //...................................................................
    ...
    // 来启用这个方法
    // !!! 这个方法将被删除
    on_do_upload: function on_do_upload(options) {
    ...
    //...................................................................
    showQuickTip: function showQuickTip(showParentAsDefault) {
      if (this.__quick_tip) {
    ...
    notification: function notification(opt) {
      if (window.Notification) {
        var popNotice = function popNotice() {
    ...
    // opt.ok     : {c}F(str)
    // opt.cancel : {c}F()
    confirm: function confirm(msgKey, opt) {
      var UI = this; // 分析参数
      ...
      $z.setUndefined(opt, "icon", '<i class="zmdi zmdi-help-outline"></i>'); // 设置图标

      if ("warn" == opt.icon) opt.icon = '<i class="zmdi zmdi-alert-triangle"></i>'; // 显示遮罩层
      ...
      seajs.use("ui/mask/mask", function (MaskUI) {
        new MaskUI(_.extend({}, opt, {
          ...
          events: {
            "click .pm-btn-ok": function clickPmBtnOk() {
              this.close();
              $z.invoke(opt, "ok", [], opt.context || this);
            },
            "click .pm-btn-cancel": function clickPmBtnCancel() {
              this.close();
          ...
          // 设置标题
          if (opt.title) this.$main.find(".pm-title").html(UI.msg(opt.title));else this.$main.find(".pm-title").remove(); // 设置按钮文字

          this.$main.find(".pm-btn-ok").html(UI.msg(opt.btnYes));
          this.$main.find(".pm-btn-cancel").html(UI.msg(opt.btnNo)).focus(); // 设置显示内容
    ...
    alert: function alert(msgKey, opt) {
      var UI = this;
      opt = opt || {}; // 分析参数
      ...
      if ("warn" == opt.icon) opt.icon = '<i class="zmdi zmdi-alert-triangle"></i>';else if ("error" == opt.icon) opt.icon = '<i class="zmdi zmdi-alert-polygon"></i>';else if ("notify" == opt.icon) opt.icon = '<i class="zmdi zmdi-notifications-active"></i>';
      $z.setUndefined(opt, "width", 400);
      $z.setUndefined(opt, "height", 240);
      $z.setUndefined(opt, "title", "i18n:info");
      $z.setUndefined(opt, "btnOk", "ok");
      $z.setUndefined(opt, "icon", '<i class="zmdi zmdi-info"></i>'); // 显示遮罩层
      ...
      seajs.use("ui/mask/mask", function (MaskUI) {
        new MaskUI(_.extend({}, opt, {
          ...
          // 设置标题
          if (opt.title) this.$main.find(".pm-title").html(UI.text(opt.title));else this.$main.find(".pm-title").remove(); // 设置按钮文字

          this.$main.find(".pm-btn-ok").html(UI.text(opt.btnOk)).focus();
          this.$main.find(".pm-btn-cancel").remove(); // 设置显示内容
    ...
    // 短暂时间提示
    // opt.callback : {c}F(str)
    toast: function toast(msgKey, opt) {
      var UI = this; // 分析参数
      ...
      if (_.isFunction(opt)) {
        ...
        opt = {
          ...
          var opt2 = {};
          if ("warn" == opt) opt2.icon = '<i class="zmdi zmdi-alert-triangle"></i>';else if ("error" == opt) opt2.icon = '<i class="zmdi zmdi-alert-polygon"></i>';else if ("notify" == opt) opt2.icon = '<i class="zmdi zmdi-notifications-active"></i>';
          opt = opt2;
        }
      ...
      seajs.use("ui/mask/mask", function (MaskUI) {
        new MaskUI(_.extend({}, opt, {
          ...
          html += '<div class="pop-msg-text">' + UI.str(msgKey) + '</div>';
          this.$main.find(".pm-body").html(html); // 不限时间
    ...
    prompt: function prompt(msgKey, opt) {
      var UI = this; // 支持直接给定 ok 的回调
      ...
      var on_ok = function on_ok() {
        ...
        if ("warn" != uiMask.$main.find(".pm-body").attr("mode")) {
          ...
          if (str) str = opt.format(str); // 关闭对话框
      ...
      seajs.use("ui/mask/mask", function (MaskUI) {
        new MaskUI(_.extend({}, opt, {
          ...
          events: {
            // 确认
            "keydown .pmp-input input": function keydownPmpInputInput(e) {
              if (13 == e.which) {
                on_ok.call(this);
              }
            },
            ...
            // 执行检查
            "input .pmp-input input": function inputPmpInputInput() {
              var uiMask = this;
              var context = opt.context || this;

              if (_.isFunction(opt.check)) {
                var str = $.trim(this.$main.find(".pmp-input input").val());
                if (str) str = opt.format(str);
                this.$main.find(".pop-msg-icon").html(opt.iconIng);
                opt.check.call(context, str, function (err) {
                  // 显示错误信息
                  if (err) {
                    uiMask.$main.find(".pm-body").attr("mode", "warn").find(".pop-msg-icon").html(opt.iconWarn);
                    uiMask.$main.find(".pmp-warn").text(UI.msg(err));
                  } // 正常回复默认 ICON
                  else {
                      uiMask.$main.find(".pm-body").attr("mode", null).find(".pop-msg-icon").html(opt.icon);
          ...
          this.$main.find(".pm-btn-cancel").html(UI.msg(opt.btnCancel)); // 设置显示内容
  ...
  var ZUI = function ZUI(arg0, arg1) {
    // 定义
    if (_.isString(arg0) && _.isObject(arg1)) {
      return ZUI.def(arg0, arg1);
    } // 获取实例
    else if (_.isElement(arg0) || arg0 instanceof jQuery) {
        return arg1 ? ZUI.checkInstance(arg0) : ZUI.getInstance(arg0);
      } // 根据 uiKey
      else if (_.isString(arg0)) {
          return arg1 ? ZUI.checkByKey(arg0) || ZUI.checkByCid(arg0) : ZUI.getByKey(arg0) || ZUI.getByCid(arg0);
        } // 未知处理
  ...
  ZUI._defer_listen = {}; // 这个函数用来定义一个 UI 模块，返回一个 Backbone.View 的类用来实例化
  ...
  ZUI.def = function (uiName, conf) {
    ...
    if (!uiDef) {
      ...
      if (conf.pkg) {
        // i18n 加载一个即可
        conf.i18n = conf.i18n || _.template("ui/{{pkg}}/i18n/{{lang}}.js")({
          pkg: conf.pkg,
          lang: "{{lang}}"
        }); // dom

        conf.dom = conf.dom || _.template("ui/{{pkg}}/{{pkg}}.html")({
          pkg: conf.pkg
        }); // TODO 这里还需要再讨论下
        ...
        var dftcss = _.template("ui/{{pkg}}/{{pkg}}.css")({
          pkg: conf.pkg
        });
      ...
      $z.setUndefined(uiBaseObj, "findDomParent", function () {
        if (!this.keepDom) return this.$el;
      }); // 定义了默认的获取 arena 方法
      ...
      uiDef = function uiDef(options) {
        // 建立自己的 ID
        this.cid = "view" + ZUI.__CID++; // 加入 Backbone 的事件支持

        _.extend(this, Backbone.Events); // 调用初始化方法
  ...
  ZUI.checkInstance = function (el) {
    ...
    if (jui.size() == 0) {
      if (console && console.warn) console.warn(el);
  ...
  ZUI.checkByCid = function (cid) {
    ...
    if (!UI) throw "UI instances 'cid=" + cid + "' no exists!";
    return UI;
  }; // 根据 uiKey 获取 UI 的实例 
  ...
  ZUI.checkByKey = function (uiKey) {
    ...
    if (!UI) throw "UI instances 'uiKey=" + uiKey + "' no exists!";
    return UI;
  }; // 异步读取全局的消息字符串
  ...
  ZUI.debug = function () {
    ...
    var _show_mark = function _show_mark(jSelf) {
    ...
    var _hide_mark = function _hide_mark(jSelf) {
      var cid = jSelf.children('b').text();
      var UI = ZUI(cid);
      if (UI) $('.ui-debug-mark', UI.el.ownerDocument).remove();
    }; // 加载处理函数
    ...
    if (!jDebugRoot.attr("bind-debug-func")) {
      ...
      jDebugRoot.on("click", ".uid-closer", function () {
        $('.ui-debug-mask').remove();
        ...
        if ($(e.target).closest('.tnd-handle, .tnd-actions').length > 0) return;
        var jSelf = $(this);
    ...
    if (inMask) $('<div class="uid-closer">').appendTo(jDebugRoot); // 绘制子节点

    _draw_sub = function _draw_sub(UI, jNode) {
      // 有子
    ...
    _draw_tree = function _draw_tree(parentUI, uiList, jSub) {
      if (!_.isArray(uiList)) return; // 循环输出

      for (var i = 0; i < uiList.length; i++) {
        // 计数
        info.count++; // 准备绘节点 DOM

        var UI = uiList[i];
        var jNode = $('<div class="uid-tnode">').appendTo(jSub);
        var jSelf = $('<div class="uid-self">').appendTo(jNode); // 查找自己的 gasket
        ...
        jSelf.attr("no-area", area ? null : "yes"); // 绘制自己的结构

        $('<span class="tnd-handle">').appendTo(jSelf);
        $('<em class="index">').appendTo(jSelf).text(i + ")"); // 有扩展点名称
        ...
        $('<div class="tnd-actions"><i class="zmdi zmdi-refresh" a="reload"></i></div>').appendTo(jSelf); // 对于 form UI，自动折叠子

        if ('ui.form' == UI.uiName) jNode.attr("collapse", "yes"); // 处理子节点
    ...
    _draw_tree(null, ZUI.tops, jDebugRoot.children(".uid-tree")); // 汇总更多统计信息


    info.instances = Object.keys(ZUI.instances).length;
    info._uis = Object.keys(ZUI._uis).length;
    info.__CID = ZUI.__CID; // 显示信息
  ...
  if (!window._zui_events_binding) {
    ...
    $(document.body).on("keyup", ZUI.on_keyup); // 全局鼠标事件

    var on_g_mouse_event = function on_g_mouse_event(e) {
      var wmm = ZUI.mousemap[e.type];
      if (wmm) for (var cid in wmm) {
        var ui = ZUI(cid);
        if (!ui) continue;
        var funcs = wmm[cid];
    ...
    $(document).contextmenu(on_g_mouse_event); // 增加 scroll/resize 事件监听
