...
define(function (require, exports, module) {
  ...
  require("theme/ui/zui.css"); // 这个逻辑用来解析 UI 的 DOM， 并根据约定，预先得到 gasket 和 layout
  // 当然 layout 不是必须有的
  ...
  var parse_dom = function parse_dom(html) {
    ...
    UI.arena = UI.$el.children('.ui-arena'); // 搜索所有的 DOM 扩展点
  ...
  var register = function register(UI) {
    ...
    if (UI.parent && UI.gasketName) {
      ...
      if (!gas) throw "Fail to found gasket '" + UI.gasketName + "' in " + UI.uiName; // 如果已经存在了插件，是否需要释放
    ...
    if (!UI.parent && UI.gasketName) {
      ...
      if (PUI) {
        gas = PUI.gasket[UI.gasketName]; // 没有就创建这个扩展点
  ...
  var ZUIPrototype = {
    ...
    initialize: function initialize(options) {
      ...
      UI.depth = UI.parent ? UI.parent.depth + 1 : 0; // 继承执行器

      UI.exec = opt.exec || (UI.parent || {}).exec; // 继承应用信息

      UI.app = opt.app || (UI.parent || {}).app; // 默认先用父类的多国语言字符串顶个先

      UI._msg_map = UI.parent ? UI.parent._msg_map : ZUI.g_msg_map; // 加载时保持隐藏

      UI.$el.attr("ui-loadding", "yes").hide(); // 调用子类自定义的 init，以及触发事件

      $z.invoke(UI.$ui, "init", [opt], UI);
      $z.invoke(opt, "on_init", [opt], UI);
      UI.trigger("ui:init", UI); // 注册 UI 实例

      register(UI);
    },
    destroy: function destroy() {
      var UI = this; //console.log("destroy: " + UI.uiName)
      // 触发事件
      ...
      UI.unwatchKey();
      UI.unwatchMouse(); // 释放掉自己

      $z.invoke(UI.$ui, "depose", [], UI); // 删除自己的 DOM 节点

      $z.invoke(UI.$el, "undelegate", []);
      UI.$el.off().remove(); // 移除注册
    ...
    render: function render(afterRender) {
      var UI = this;
      var options = UI.options; // 确定语言

      UI.lang = (UI.parent || {}).lang || window.$zui_i18n || "zh-cn"; //============================================== i18n读完后的回调
      // 看看是否需要异步加载多国语言字符串
      ...
      var callback = function callback() {
        ...
        var do_after_redraw = function do_after_redraw() {
          ...
          window.setTimeout(function () {
            ...
            UI.trigger("ui:display", UI); // 让 UI 的内容显示出来

            UI.$el.removeAttr("ui-loadding").show(); // 因为重绘了，看看有木有必要重新计算尺寸，这里用 setTimeout 确保 resize 是最后执行的指令
    ...
    // 汇报自己完成了哪个延迟加载的 UI
    defer_report: function defer_report(i, uiType) {
      var UI = this;
      if (!UI._loaded_uiTypes) UI._loaded_uiTypes = []; // 如果仅仅给了个 uiType，那么自动寻找 i
    ...
    _check_defer_done: function _check_defer_done() {
      var UI = this;
    ...
    resize: function resize(deep) {
      ...
      if (UI.arena) {
        // 需要调整自身，适应父大小
        if (UI.options.fitparent === true || UI.options.fitparent !== false && UI.arena.attr("ui-fitparent")) {
          // 调整自身的顶级元素
          var w, h;
    ...
    */
    watchKey: function watchKey(which, keys, handler) {
      ...
      $z.pushValue(ZUI.keymap, [key, this.cid], handler); // 记录到自身以便可以快速索引
    ...
    unwatchKey: function unwatchKey(which, keys) {
      ...
      var key = this.__keyboard_watch_key(which, key); // console.log("unwatchKey : '" + key + "' : " + this.cid);
      // 删除全局索引
    ...
    // 得到多国语言字符串
    msg: function msg(key, ctx, msgMap) {
    ...
    showLoading: function showLoading(selector) {
    ...
    subUI: function subUI(uiPath) {
      ...
      for (var i = 0; i < ss.length; i++) {
        var s = ss[i];
        var g = UI.gasket[s];
        if (!g || g.ui.length == 0) return null;
        UI = g.ui[0];
      }
    ...
    // !!! 这个方法要作废 >o< !!!
    eval_obj_display: function eval_obj_display(obj, displayMap) {
      if (!_.isObject(obj)) return;
      if (!_.isObject(displayMap)) return;
      obj._display = {};
    ...
    get_obj_val_by: function get_obj_val_by(obj, ref) {
      var val = obj[ref.key];
    ...
    //............................................
    // 监听本 UI 的模块事件
    listenModel: function listenModel(event, handler) {
    ...
    if (UI) {
      ...
      for (var key in UI.gasket) {
        ...
        if (G.ui && G.ui.length == 1) {
          str += "\n" + prefix + "  @" + $z.alignLeft('"' + key + '"', 8, " ") + "-> " + ZUI.dump_tree(G.ui[0], depth + 1);
  ...
  ZUI.def = function (uiName, conf) {
    ...
    if (!UIDef) {
      ...
      _.extend(viewOptions, ZUIPrototype); // 注册
  ...
  if (!window._zui_events_binding) {
    ...
    $(document).contextmenu(on_g_mouse_event); // TODO 捕获 ondrop，整个界面不能被拖拽改变
    // 标记以便能不要再次绑定了
