...
(function ($, $z) {
  ...
  var zRect = {
    ...
    */
    gen: function gen(ele, opt) {
      // 得到目标
      var jEle = $(ele); // 目标必须有元素，否则返回 null
      ...
      $z.setUndefined(opt, "padding", 0); // 准备返回值

      var rect; // 如果计算 body 或者 document 或者 window

      var ele = jEle[0]; // 可能是 document 或 window
      ...
      if (!ele.ownerDocument) {
        rect = $z.winsz(ele);
          ...
          rect = $z.winsz(jEle[0].ownerDocument.defaultView);
            ...
            if (opt.scroll_c) {
              var $doc = $(jEle[0].ownerDocument); //console.log("scrollTop/Left", $doc.scrollTop(), $doc.scrollLeft());

              rect.top -= $doc.scrollTop();
              rect.left -= $doc.scrollLeft(); //console.log("rect:", this.dumpValues(rect));
            } // 包括外边距
            ...
            if ("margin" == opt.boxing) {
              ...
              rect.left -= $z.toPixel(style.marginLeft);
                ...
                var style = window.getComputedStyle(jEle[0]);
                rect.top += $z.toPixel(style.paddingTop) + $z.toPixel(style.borderTopWidth);
                rect.left += $z.toPixel(style.paddingLeft) + $z.toPixel(style.borderLeftWidth);
                ;
              } // 默认就是 border-box
              else {
      ...
      if (opt.overflow) {
        var scrollbar = $z.scrollbar(opt.overflowEle || jEle); // 水平滚动条导致高度减小
    ...
    isRect: function isRect(rect) {
      return rect && _.isNumber(rect.top) && _.isNumber(rect.left) && _.isNumber(rect.width) && _.isNumber(rect.height) && _.isNumber(rect.right) && _.isNumber(rect.bottom);
    },
    //.............................................
    // 根据四个数值，创建一个矩形
    //  - values : 包含四个数值的数组，如果是字符串，会被分隔
    //  - mode   : 数值的意义 tlwh,tlbr,brwh,xywh
    create: function create(values, mode) {
      // 没值就返回空
      if (!values) return null; // 处理输入值
      ...
      for (var i = 0; i < values.length; i++) {
        ...
        if (isNaN(v)) {
          throw "zdimension.rect.create: invalid values[" + i + "] : " + values[i];
        }
    ...
    //.............................................
    quick: function quick(values, keys, dft) {
      var re = {};
      ...
      if (keys) {
        ...
        for (var i = 0; i < len; i++) {
          ...
          var val = values[i];
          if (_.isUndefined(val) || _.isNull(val)) val = dft;
          if (!_.isUndefined(val) && !_.isNull(val)) re[key] = val * 1;
        }
      }
    ...
    pick: function pick(rect, keys, dft) {
      var re = {};
      ...
      if (keys && _.isString(keys)) {
        ...
        for (var i = 0; i < keys.length; i++) {
          ...
          var key = this.__quick_key_map[k] || k;
          var val = rect[key];
          if (_.isUndefined(val)) val = dft;
          if (!_.isUndefined(val)) re[key] = val;
        }
      }
    ...
    //.............................................
    // 快速精简的矩形信息，以便人类查看
    dumpValues: function dumpValues(rect, keys, precise) {
      if (!rect) return "-nil-";
      keys = keys || "ltwh";
      var info = {
        t: rect.top,
    ...
    //.............................................
    // 快速精简的点信息，以便人类查看
    dumpPos: function dumpPos(pos) {
      if (!pos) return "-nil-";
      return $z.tmpl("x:{{x}},y:{{y}}")(pos);
    },
    //.............................................
    // 自动根据矩形对象的值进行填充
    autoCount: function autoCount(rect, quiet) {
      // 有 width, height
      if (_.isNumber(rect.width) && _.isNumber(rect.height)) {
        if (_.isNumber(rect.top) && _.isNumber(rect.left)) return this.count_tlwh(rect);
        if (_.isNumber(rect.bottom) && _.isNumber(rect.right)) return this.count_brwh(rect);
        if (_.isNumber(rect.x) && _.isNumber(rect.x)) return this.count_xywh(rect);
      } // 有 top,left
      else if (_.isNumber(rect.top) && _.isNumber(rect.left)) {
          if (_.isNumber(rect.bottom) && _.isNumber(rect.right)) return this.count_tlbr(rect);
        } // 不知道咋弄了


      if (!quiet) throw "Don't know how to count rect:" + $z.toJson(rect);
      return rect;
    },
    //.............................................
    // 根据 top,left,width,height 计算剩下的信息
    count_tlwh: function count_tlwh(rect) {
    ...
    count_tlbr: function count_tlbr(rect) {
    ...
    count_brwh: function count_brwh(rect) {
    ...
    count_xywh: function count_xywh(rect) {
      var W2 = rect.width / 2;
    ...
    asCss: function asCss(rect, vpWidth, vpHeight, keys) {
    ...
    //  - y : 点 Y 周坐标
    // 返回新的相对于矩形的点对象
    ccs_point_tl: function ccs_point_tl(rect, x, y) {
      // 支持对象作为数据参数
    ...
    // 也可以是 {x,y} 格式的对象
    // 默认为 {x:0,y:0} 
    relative: function relative(rect, base, forCss, baseScroll) {
      // 计算 base 的滚动
    ...
    // 返回矩形自身
    zoom: function zoom(rect, vp, zoomX, zoomY) {
      vp = vp || rect;
      zoomY = zoomY || zoomX;
    ...
    // 返回矩形自身
    zoomScale: function zoomScale(viewport, rect, forContains) {
    ...
    // - tY   : Y 周位移
    // 返回矩形自身
    translate: function translate(rect, tX, tY) {
      // 支持对象作为数据参数
    ...
    X- <--left----center----right---> X+
     ...
     - space   : 停靠的间距 {X: 0,      Y: 0}
                 如果停靠在中点(center) 则不管用
    */
    dockIn: function dockIn(rect, target, axis, space) {
      // 参数默认值
      axis = axis || {
        X: "left",
        Y: "top"
    ...
    union: function union() {
      var rects = $z.toArgs(arguments, true); // 空
    ...
    //.............................................
    // 相并面积
    union_area: function union_area() {
    ...
    overlap: function overlap() {
      var rects = $z.toArgs(arguments, true); // 少于1个
    ...
    //.............................................
    // 相交面积
    overlap_area: function overlap_area() {
      var rects = $z.toArgs(arguments, true);
    ...
    //.............................................
    // A 是否与 B 相交
    is_overlap: function is_overlap(rectA, rectB) {
    ...
    // rectB   : 用来限制原始矩形的边界矩形
    // overlap : 表示实在放不下了就剪裁
    boundaryIn: function boundaryIn(rectA, rectB, overlap) {
      var re = {}; // @移动上下边
      // 在上面，先修改 top
      ...
      if (overlap) return this.overlap(re, rectB); // 直接返回
    ...
    move_xy: function move_xy(rect, pos) {
      rect.x = pos.x;
      rect.y = pos.y;
    ...
    move_tl: function move_tl(rect, pos, offset) {
  ...
  var zDom = {
    //.............................................
    // 获得某元素的某真实属性
    getProp: function getProp(el, key) {
      var sty = window.getComputedStyle($(el)[0]); //console.log(sty);
      // 一组属性
    ...
    getRootFontSize: function getRootFontSize(doc) {
      doc = doc || document;
    ...
    // 否则获取这个元素的大小
    getMeasureConf: function getMeasureConf(docOrEle) {
      var jq = $(docOrEle);
      var doc = jq[0];
      var conf; // 给的就是文档
      ...
      if (doc.defaultView) {
        ...
        conf.baseSize = this.getRootFontSize(doc);
          ...
          conf = this.winsz(doc, true); // 准备给定元素的矩形

          var rect = zRect.gen(jq, {
            boxing: "content",
            overflow: true
          });
          conf.width = rect.width; // 附件 baseSize
    ...
    */
    toMeasureNum: function toMeasureNum(key, val, conf) {
      // 首先分析一下值
    ...
    */
    toMeasureStr: function toMeasureStr(key, n, conf) {
    ...
    //.............................................
    // 获得视口的矩形信息
    winsz: function winsz(win, onlyWidthHeight) {
      ...
      win = win || window; // 哦是 document 对象，转 window

      if (win.defaultView) win = win.defaultView; // 来吧
      ...
      if (onlyWidthHeight) return rect; // 继续剩余的值
