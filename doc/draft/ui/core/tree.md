...
```
{
    ...
    // 根据一个对象的 id 获取该对象的所有祖先节点
    // 给出一个树节点的 ID，函数需要传递给回调一个该节点的祖先数组（不包括该节点）
    // callback 的格式是 F([祖先节点]); 
    // 函数接受 id 参数必须为字符串
    // 「场景」
    // 因为如果想 setActived 一个树节点，但是这个节点现在没有被展开，那么
    // 就需要调用这个函数，获取这个节点的层级信息，然后一层层展开。
    // 本选项不设置的话，创建的树实例，就没有这种特性
    ancestor : {c}F(id, callback);
