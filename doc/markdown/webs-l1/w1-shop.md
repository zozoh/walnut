---
title: 商城模型
author: zozohtnt@gmail.com
key: w1-shop
---

--------------------------------------
# 动机：为什么要有商城套件

商城套件给出一个维度，从商城的角度来看 `Walnut` 提供的各种能力。
这个文档只是一篇综述，如果你想构建一个商城系统，
通过这篇文档，可以比较清晰的了解`Walnut` 是如何提供相应支持的

--------------------------------------
# 计划应用场景

- PC/手机移动网页商城
- 微信小程序商城
- 手机APP商城

--------------------------------------
# 设计思路与边界

`Walnut`提供的服务器后端能力包括：

- 订单/支付/优惠券体系
- 购物车/收藏商品
- 通用的数据管理
  + 商品
  + 优惠券
  + 收货地址
  + 库存管理
- 用户购买统计（暂未实现）
- 物流配送信息整合（暂未实现）

同时，根据[通用Web接口][w0-api]，如果搭建好服务器端，
你的应用几乎不需要编写服务器端代码。

--------------------------------------
# 数据结构描述

一个商城，我们提供如下的数据实体：

 Name           | Description
----------------|--------------
`product`       | 商品/产品
`order`         | 订单
`coupon`        | 优惠券
`payment`       | 支付单
`invoice`       | 发票
`freight_sheet` | 运费表
`address`       | 收货地址
`basket`        | 购物车

--------------------------------------
## `product`商品/产品

```bash
id: ID         # 商品唯一ID
nm: "xxx-xx"   # 可用作商品人类可识别的唯一编号
cate: "xxx"    # 商品的分类名称
#-----------------------------
# 展示
title: "xxx"   # 商品标题
#-----------------------------
# 库存(表示可以售卖的商品)
#-----------------------------
sku: 100
#-----------------------------
# 价格
#-----------------------------
# 通常一个商城里的商品都是统一的货币单位
# 因此在商品记录里，不宜设置货币单位否则容易坑
# 当然这个字段也不是必须的，譬如用
# pro_id + price_by 则可以应用动态价格规则
price: 35.68   # 价格（元）
#-----------------------------
# 重量（公斤）（可作为运费计算依据）
# 如果0或者负数，则表示不记入运费
weight  : 1.5
#-----------------------------
# 固定运费（元）
# 表示特殊商品，运费一律固定一个价格
# 如果未设定或为0或负数，则表示需要计算运费
freight : 12.5
#-----------------------------
# 库存（表库存单位）
sku : 1000
#-----------------------------
# 【选】采用动态价格规则
# 其中，价格对象具体采用哪个键存放规则列表
# 这个应该是从命令参数里得到的
pro_id : ID          # 价格规则对象的 ID
price_by : "rule00"  # 价格规则名称
```

商品为了展示，当然可以增加更多的元数据，但是`商城模型`不关心。

> 更多，请参看[Thing·数据结构][th0-data]


--------------------------------------
## `priceRule` 价格规则

```js
[
   {
      "name": "rule001",
      "title": "标题001",
      "rules": [
         {
            "sku": "[1,9]",
            "price": 200
         },
         {
            "sku": "[10,99]",
            "price": 150
         },
         {
            "sku": "[100,)",
            "price": 180
         }
      ]
   },
   {
      "name": "rule002",
      "title": "标题002",
      "rules": [
         {
            "sku": "[1,9]",
            "price": 800
         },
         {
            "sku": "[10,99]",
            "price": 500
         },
         {
            "sku": "[100,)",
            "price": 300
         }
      ]
   }
]
```

用作商品的动态价格，即满足一定条件的商品，才会应用对应的价格。
现在，条件仅仅支持购买数量`sku`

--------------------------------------
## `order`订单

```bash
id: ID         # 【自】订单唯一ID
#-----------------------------
# 订单类型
#
#  - A: 普通订单，会执行下单->支付->发货->完成这一标准步骤
#  - Q: 简单订单，适用于虚拟物品，支付成功就直接完成
#  - S: 父订单，如果订单被自动拆分，那么将
#           - 在父订单完成支付，子订单结算运费
#           - 运单追踪将在子订单完成
#
# 默认"A"
#-----------------------------
tp: "A"
porder : ID           # 【自】父订单 ID
sub_orders : [ID,ID], # 【自】子订单列表
#-----------------------------
# 【必】商品列表
#-----------------------------
products: [{
  id : ID,      #【必】商品 ID 
  amount:3      #【必】购买数量
  title :"xxx", #【查】商品名
  price :29.9,  #【自】商品订单价格（元）
  weight : 10.23,  #【查】重量（公斤）（可作为运费计算依据）
  pcount: 53,          #【自】同一价格体系下的商品购买数量
  subtotal: 4500.0,    #【自】小计 (price x amount)
  freight : 5,         #【查】固定运费（元）
  pro_id : ID          #【查】价格规则对象的 ID
  price_by : "rule00"  #【查】价格规则名称
}]
#-----------------------------
# 【冗】根据商品列表自动生成，第一个商品的ID
proid0: ID
# 【冗】根据商品列表自动生成，全部商品的ID
proids: [ID, ID, ID ...]
# 【冗】本订单共包括的商品数量（商品列表所有商品数量之和）
pro_c : 3
#-----------------------------
# 物流信息
# > ! 站点必须关联地址库
#-----------------------------
# 卖家发货地址
# 默认地址可以取得自 {tp:"S", dftaddr:true}
addr_ship_country : "CN",             # 国家编码，默认 CN
addr_ship_code    : "110108017000",   # 12位地址编码
addr_ship_door    : "xxx xx A-302",   # 详细到门牌的地址
#-----------------------------
# 用户收货地址
# 默认地址可以取得自 {tp:"U", dftaddr:true, uid:"xx"} 
addr_user_country : "CN",             # 国家编码，默认 CN
addr_user_code    : "110108017000",   # 12位地址编码
addr_user_door    : "xxx xx A-302",   # 详细到门牌的地址
user_name  : "xxx"      # 联系人姓名
user_phone : "139.."    # 联系人手机
user_email : "zz@z.com" # 联系人邮箱

# 下面的是地址的冗余信息，通常可以通过 addr_user_code 查表得到
addr_user_province : "Beijing"  # 省/直辖市
addr_user_city     : "BeiJing"  # 城市
addr_user_area     : "BeiJing"  # 区县
addr_user_street   : "xxx"      # 乡镇/街道
#-----------------------------
# 财务信息
#-----------------------------
# 关联的发票对象
# 记录一个完整的发票信息，参见发票表
invoice : {...}
#-----------------------------
# 【选】优惠券列表
#-----------------------------
coupons: [{
  id : ID,        # 【必】优惠券 ID
  title : "xxx",  # 【查】优惠券名
  type  : 1,      # 【查】类型，1:代金券，2:折扣券
  value : 20,     # 【查】代金券：元；折扣券0.0~1.0浮点
  thres : 200,    # 【查】订单满足这个金额方可应用，0表示不限
  expi  : AMS     # 【查】过期时间
}]
#-----------------------------
# 【选】摘要信息
#-----------------------------
# 默认为支付商户的名称
title: "xxx"     # 订单标题
note : "xxx"     # 订单备注
#-----------------------------
# 【查】卖家名称（默认为支付配置目录名）
# 根据pay_tp读站点设置
#-----------------------------
seller: "theName"
#-----------------------------
# 买家信息
#-----------------------------
accounts : ID   # 【自】买家用户库ID
buyer_id : ID   # 【自】买家ID，通常从当前会话中获取
#-----------------------------
# 价格信息
#-----------------------------
# 本订单商品汇总的价格（优惠前）
total: 245.12
# 本订单所需运费（元）
freight: 23.5
# 本订单实际优惠了多少（元）
# 根据优惠券自动计算
discount: 16.8
#-----------------------------
# 支付信息
#-----------------------------
# 订单总金额 = total + freight
# 支付总金额 = total + freight - discount
price  : 321.56       # 【查】订单总金额（包括运费）
fee    : 319.12       # 【查】优惠后金额，用来实际支付
pay_tp : "wx.qrcode"  # 【必】支付类型
pay_id : ID           # 【自】支付单ID 
currency : :"RMB"     # 【选】订单的货币单位，默认RMB
#-----------------------------
# 状态信息【自】
#  - NW : 新建
#  - WT : 等待支付（已经关联了支付单）
#  - OK : 买家已经支付成功
#  - FA : 买家已经支付失败
#  - SP : 卖家已发货
#  - DN : 订单完成
#-----------------------------
or_st : "NW"
#-----------------------------
# 时间戳
#-----------------------------
ct : AMS       # 【自】创建时间
lm : AMS       # 【自】最后变动时间
wt_at : AMS    # 【自】创建支付单时间
ok_at : AMS    # 【自】买家支付成功时间
fa_at : AMS    # 【自】买家支付失败时间
sp_at : AMS    # 【自】卖家已发货时间
dn_at : AMS    # 【自】订单完成时间
```

- `【查】`: 表示该字段的值是自行查询获得
- `【自】`: 表示该字段的值是自动生成
- `【必】`: 表示该字段是从客户端请求中读取的，所以不能缺少
- `【选】`: 表示该字段是从客户端请求中读取的，如果不指定则会有默认值

--------------------------------------
## `coupon`优惠券

```bash
id    : ID       # 优惠券ID
title : "xxx"    # 优惠券名称
owner : ID       # 所属用户 ID
#-----------------------------
# 类型，1:代金券，2:折扣券
type  : 1
#-----------------------------
# 根据类型有不同的意义：
# 代金券：表示元
# 折扣券: 表示折扣，值应该是`0.0 ~ 1.0`的浮点
value : 40
# 订单满足这个金额方可应用，0表示不限
#-----------------------------
# 使用条件:
# 订单满足这个金额方可应用，0表示不限
thres : 0
# 过期时间（绝对毫秒数）
# 0 表示永久有效
cpn_expi : AMS
```

--------------------------------------
## `payment`支付单

这个请参看[通用支付模型][f1-pay]的文档，这里不另行赘述。

--------------------------------------
## `invoice`发票

```bash
id  : ID           # 发票唯一 ID
ct  : AMS          # 创建时间
uid : ID          # 关联用户 ID
#--------------------------------------
# 发票类型
#  - 1 : 个人普通发票
#  - 2 : 企业普通发票
#  - 3 : 增值税专用发票
inv_type : 1
#--------------------------------------
#
# 收票人信息
#
#--------------------------------------
u_name  : "xxx",        # 收票人姓名
u_phone : "139.."       # 收票人手机
u_email : "x@x.x"       # 收票人邮箱
#--------------------------------------
#
# 发票信息
#
#--------------------------------------
inv_title  : "xxx",     # 发票抬头
inv_tfn    : "xxx"      # 发票税号
inv_dft    : false      # 是否是默认抬头
#--------------------------------------
#
# 银行信息
#
#--------------------------------------
bank_name : "xxx"       # 开户行
bank_account : "xxx"    # 银行账户
#--------------------------------------
#
# 企业信息
#
#--------------------------------------
busi_addr  : "xxx"      # 企业地址
busi_phone : "xxx"      # 企业电话 
```

--------------------------------------
## `freight_sheet`运费表

> 运费表是一个 JSON 文件

```js
{
  //-----------------------------
  // 全局配置
  "first_weight"     : 1,    // 首重重量（公斤）默认 1
  "additional_unit"  : 0.5,  // 续重的单位（公斤）默认 1
  //-----------------------------
  // 规则列表
  // 键：国家编码（例如 CN），值：运费规则数组
  "rules": {
    "CN": [{
      // 规则标题，仅仅是助记用的
      "title" : "北京至全国"，
      //-----------------------------
      // 发货地（六位地址编码）
      // !注: 000000 表示全国
      "ship_code" : "110000",
      //-----------------------------
      // 目的地址（六位地址编码）
      // !注: 000000 表示全国
      "target_code" : "000000",
      //-----------------------------
      // 运费标准
      "first"        : 10.5,  // 首重价格（元）
      "additional"   : 0.8    // 续重价格（元）
    }]
  }
}
```

--------------------------------------
## `address`收货地址

```bash
id  : ID      # 唯一ID
#-----------------------------
# 地址类型
#  - U : 普通用户（买家）地址
#  - S : 商家用户（卖家）地址
tp  : "U"
#-----------------------------
# 默认地址
# 一个用户只能有一个默认地址
dftaddr : false
#-----------------------------
uid : ID      # 用户ID
#-----------------------------
# 国家统一编码，大写
# 命令 `lbs countries` 可以查看全部国家编码
# 默认，会认为是 "CN" 表示中国
# 不同的国家有不同的地址编码系统
country: "CN"
# 地址编码，不同国家有不同的规则
# 以中国为例，为 12位地址码
# 如果是欧美地区，应该就是邮编
code: "110108017000"
#-----------------------------
# 自由输入
postcode : "100085"   # 邮编
province : "Beijing"  # 省/直辖市
city     : "BeiJing"  # 城市
area     : "BeiJing"  # 区县
street   : "xxx"      # 乡镇/街道
door     : "xxx"      # 详细到门牌
#-----------------------------
# 联系人
consignee : "xxx"      # 联系人姓名
phone     : "139.."    # 联系人手机
email     : "zz@z.com" # 联系人邮箱
```

--------------------------------------
## `basket`购物车

购物车数据返回的总是一个数组，它是由`Redis`持有的一个`有序集合`。
通过命令 `man buy` 可以看到更多的细节。

```js
[{
  name: "67a2..r6y1",   // 商品ID
  count: 3,             // 数量
  obj: {..}             // 商品的元数据
}]
```

--------------------------------------
# 相关知识点

- [基础账户授权模型][c0-bam]
- [核心会话服务][c0-css]

[c0-bam]: ../core-l0/c0-baice-auth-model.md
[c0-css]: ../core-l0/c0-core-session-service.md
[c0-pvg]: ../core-l0/c0-pvg-basic.md
[c2-pvg]: ../core-l2/c2-pvg-more.md
[f0-wxp]: ../func-l0/f0-weixin-payment.md
[f0-zfb]: ../func-l0/f0-alipay.md
[f1-pay]: ../func-l1/f1-payment.md
[th0-data]: ../thing-l1/th1-data.md
[w0-api]: ../webs-l0/w0-api-overview.md