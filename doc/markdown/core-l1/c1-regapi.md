...
**!!!注意**，缓存策略仅在`HTTP GET`才有效，其他`HTTP`方法的`API`都会无视
任何缓存的设置。
...
# 尝试命中缓存。
# 这个选项就是一个 WnValidate 的验证器。验证的对象是 Query String 组成
...
```bash
# 这个是缓存的开关，任何缓存都必须是一个文件
# 没有声明这个参数，缓存策略无效
# 包括上面的请求检查等，会统统跳过
# 这个路径是一个字符串模板，渲染上下文为整个请求对象
# 因此，你可以使用诸如 ${http-qs-xxx} 等占位符
# 同时，会多一个占位符 ${http-query-finger} 表示 HTTP 请求的指纹
cache-path : "~/.domain/cache/api/get-${http-qs-id}.json",
# 可选测试，如果没有指定，则用缓存文件的 mime 作为
# 响应的 Content-Type
cache-mime : "text/json"
```
