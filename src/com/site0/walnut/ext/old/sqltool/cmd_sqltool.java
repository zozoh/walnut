package com.site0.walnut.ext.old.sqltool;
...
public class cmd_sqltool extends JvmHdlExecutor {

    @Override
    protected void _find_hdl_name(WnSystem sys, JvmHdlContext hc) {
        // 如果第一个参数就是处理器，那么，HOME 则自动寻找
        if (hc.args.length < 1) {
            throw Er.create("e.cmd.sqltool.lackArgs", hc.args);
        }
        ...
        // 第一个参数必须为SqlTool目录名
            ...
            // 获取主目录
                ...
                String aph = Wn.normalizeFullPath("~/.sqltool/" + hc.args[0], sys);
                hc.oRefer = sys.io.check(null, aph);
            }

            if (hc.oRefer != null && hc.oRefer.isLink()) {
                String aph = Wn.normalizeFullPath("~/.sqltool/" + hc.oRefer.link(), sys);
                hc.oRefer = sys.io.check(null, aph);
            }
        ...
        // 得到配置文件
        if (null != hc.oRefer) {
            WnObj oConf = sys.io.check(hc.oRefer, "dataSource");
            hc.setv("dataSource_obj", oConf);
            hc.setv("dataSource_conf", sys.io.readJson(oConf, NutMap.class));
        }
    }
}
