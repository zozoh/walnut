package com.site0.walnut.ext.sys.crontab;
...
import com.site0.walnut.cron.CronEach;
...
public class WnCronService extends NutRunner {
    ...
    public long exec() throws InterruptedException {
        ...
        for (CronJob cronJob : jobs) {
            ...
            cronJob.quartz.each(mats, new CronEach<JobTime>() {
                public void invoke(JobTime[] array, int index)
                        throws Exception {
                    if (array[index] == null) {
                        array[index] = new JobTime();
                        array[index].timeAt = (index) * timer_int * 1000
                                              + dayOfBegin
                                              + 99;
        ...
        for (JobTime jobTime : jobTimes) {
            ...
            if (diff < 0) {
                log.debugf("已经过了任务执行时间 %s",
                           Times.sDT(new Date(jobTime.timeAt)));
                continue;
            }
            log.debugf("当前时间 %s 下一个任务的执行时间 %s",
                       Times.sDT(new Date(timenow)),
                       Times.sDT(new Date(jobTime.timeAt)));
    ...
    public List<CronJob> _reload() {
        ...
        for (int i = 0; i < tmp.length; i++) {
            ...
            line = line.trim();
            if (line.startsWith("###")) {
                job = Wlang.map2Object(Wlang.map(line.substring(3)),
                                       CronJob.class);
