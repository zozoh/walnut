...
public class redis_decrby extends RedisFilter {
    @Override
    protected void process(WnSystem sys, RedisContext fc, ZParams params) {
        String key = params.val_check(0);
        long decrement = params.val_check_long(1);
        fc.addHandler(new RedisHanlder("DECRBY", key) {
            public Object run(RedisContext fc, Jedis jed) {
                return jed.decrBy(key, decrement);
            }
        });
    }
}