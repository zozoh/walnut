...
import static com.site0.walnut.ext.data.archive.util.Archives.*;
...
public class TgzArchiveExtracting implements ArchiveExtracting {

    private InputStream ins;
    private TarArchiveInputStream tar;
    private GZIPInputStream gzip;

    public TgzArchiveExtracting(InputStream ins) {
        this(ins, null);
    }

    public TgzArchiveExtracting(InputStream ins, Charset charset) {
        if (null == charset) {
            charset = Encoding.CHARSET_UTF8;
        }
        this.ins = ins;
        try {
            this.gzip = new GZIPInputStream(ins);
            this.tar = new TarArchiveInputStream(gzip, charset.name());
        }
        catch (IOException e) {
