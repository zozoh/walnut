...
public class AppInitItem {
    ...
    private AppInitItemContentType contentType;
    ...
    final static Log log = Wlog.getCMD();
        ...
        if (obj instanceof AppInitItem) {
            ...
            if (this.hasContentFilePath()) {
                if (item.hasContentFilePath()
                    && !contentFilePath.equals(item.contentFilePath)) {
                    return false;
                }
    ...
    public void addMeta(String json, NutBean vars) {
        if (!Strings.isBlank(json)) {
            ...
            try {
                ...
                log.warnf("Fail to parse json: %s", json);
                throw Er.wrap(e);
            }
        }
    }
    ...
    public AppInitItemContentType getContentType() {
        return contentType;
    }
    ...
    public void setContentType(AppInitItemContentType contentType) {
        this.contentType = contentType;
    }

    public void setContentTypeByStr(String ct) {
        String ctu = ct.toUpperCase();
        this.contentType = AppInitItemContentType.valueOf(ctu);
    }

    public boolean isContentAsTmpl() {
        return AppInitItemContentType.TMPL == this.contentType;
    }

    public boolean isContentAsCopy() {
        return AppInitItemContentType.COPY == this.contentType;
    }

    public boolean isContentAsExplain() {
        return AppInitItemContentType.EXPLAIN == this.contentType;
    }
