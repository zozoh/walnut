...
public class WnObjEntityGenerating {
	...
	private void __setup_default_indexes(WnObjEntity en) {
		...
		if (null == indexes || indexes.isEmpty()) {
			...
			if (null != en.getField("id")) {
				indexes.add(Wlang.map("unique:true").setv("name", "obj_id").setv("fields", Wlang.list("id")));
			}

			// 有 PID 和 NM
			if (null != en.getField("pid") && null != en.getField("nm")) {
				indexes.add(
						Wlang.map("unique:true").setv("name", "obj_pid_nm").setv("fields", Wlang.list("pid", "nm")));
			}
	...
	private void __add_indexes_to_entity(WnObjEntity en) {
		for (NutMap map : conf.getIndexes()) {
			...
			for (String fnm : eiFields) {
				...
				if (null == ef) {
					throw Er.create("e.io.dao.entity.index.FieldNotDefined", fnm + "@" + ei.getName());
				}
				fields.add(ef);
			}
			ei.setFields(fields);
	...
	private void __fill_field_default_value(NutMap fld) {
		...
		// 设置默认值
		if (fld.has("defaultValue")) {
			Object dft = fld.get("defaultValue");
			fld.put("defaultValue", new CharSegment(dft.toString()));
		}
