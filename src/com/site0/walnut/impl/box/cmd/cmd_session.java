...
public class cmd_session extends JvmExecutor {
    ...
    public void exec(WnSystem sys, String[] args) throws Exception {
        ...
        if (params.has("site") || params.has("host") || params.is("list")) {
            ...
            // 系统会话列表，只有管理员能干
            if (params.is("list")) {
                beans = __load_as_sys(sys, limit);
            }
            // 站点会话列表，只有对应站点管理员能干
            else {
                beans = __load_as_site(sys, params, limit);
            }
    ...
    protected List<NutBean> __load_as_sys(WnSystem sys, int limit) {
        List<NutBean> beans;
        WnUser me = sys.getMe();
        WnRoleType myRole = sys.roles().getRoleTypeOfGroup(me, "root");
        if (!myRole.isOf(WnRoleType.ADMIN)) {
            throw Er.create("e.cmd.session.list.forbid", "Should be ADMIN of root");
        }
        List<WnSession> seList = sys.auth.querySession(limit);
    ...
    protected List<NutBean> __load_as_site(WnSystem sys, ZParams params, int limit) {
        ...
        if (!site.isRoleOfHome(sys, WnRoleType.ADMIN)) {
            throw Er.create("e.cmd.session.list.forbid", "Should be ADMIN of site:" + sitePath);
        }

        // 读取列表
        List<WnSession> seList = auth.querySession(limit);
    ...
    private void output_as_table(WnSystem sys, ZParams params, List<NutBean> beans) {
