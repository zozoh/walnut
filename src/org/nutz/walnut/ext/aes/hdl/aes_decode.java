...
public class aes_decode implements JvmHdl {
	 ...
	 * @param text 需要解密的密文
	 * @return 解密得到的明文
	 * @throws AesException axes解密失败
	 */
	static public byte[] decrypt(String cipherStr, String textBase64, String aesKeyBase64, String ivBase64) throws AesException {
		try {
			// 设置解密模式为AES的CBC模式
	...
	static public byte[] decrypt(String cipherStr, byte[] encrypted, byte[] aeskey, byte[] _iv) throws AesException {
		try {
			// 设置解密模式为AES的CBC模式
			Cipher cipher = Cipher.getInstance(cipherStr);
