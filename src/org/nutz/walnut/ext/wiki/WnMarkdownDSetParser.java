...
public class WnMarkdownDSetParser extends AbstractMarkdownDSetParser {
    ...
    protected void checkPath(NutDSet home, String path) {
        // 得到主目录
        WnObj dHome = io.check(null, path);
    ...
    protected NutMap loadConfig(NutDSet home, String path, String configPath) {
        // 得到主目录
        WnObj dHome = (WnObj) home.getPrimerObj();

        // 试图得到配置信息
        NutMap conf = new NutMap();
        WnObj fConf = io.fetch(dHome, Strings.sBlank(configPath, "zdoc.conf"));
        if (fConf != null) {
            conf = io.readJson(fConf, NutMap.class);
        }
