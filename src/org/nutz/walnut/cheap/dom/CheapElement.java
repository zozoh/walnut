...
public class CheapElement extends CheapNode {
	...
	public void cloneChildrenTo(CheapElement el) {
		// Clone Children
		if (this.hasChildren()) {
	...
	@Override
	public CheapElement cloneNode() {
		CheapElement el = this.cloneSelf();
		this.cloneChildrenTo(el);
		return el;
	}
	...
	public String toBrief() {
		...
		// 下标
		sb.append(String.format("[%d]%s(%d): ", this.getNodeIndex(), tagName, this.getElementIndex()));
		// 标签
		if (null != className) {
			sb.append('.');
	...
	/**
	 * @param regex 标签名的正则表达式。标签名全部用大写形式匹配
	 * @return 是否符合标签
	 */
