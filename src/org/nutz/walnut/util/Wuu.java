...
public abstract class Wuu {
    ...
    private static Random _RA = new Random(System.currentTimeMillis());
    ...
    public static int random(int min, int max) {
        int bou = Math.abs(max - min);
        return _RA.nextInt(bou) + min;
    }
    ...
    public static long random(long min, long max) {
        long bou = Math.abs(max - min);
        double d = _RA.nextDouble();
        long n = (long) (bou * d);
        return min + n;
    }
    ...
    public static String UU16(UUID uu) {
        return Ws.padStart(Long.toHexString(uu.getMostSignificantBits()), 16, '0')
               + Ws.padStart(Long.toHexString(uu.getLeastSignificantBits()), 16, '0');
    }
