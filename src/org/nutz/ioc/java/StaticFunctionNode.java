...
public class StaticFunctionNode extends ChainNode {
    ...
    public StaticFunctionNode(String className, String name, ChainNode[] args) {
        try {
            Mirror<?> mirror = Mirror.me(Wlang.loadClass(className));
            if (null == args || args.length == 0) {
                try {
                    method = mirror.getGetter(name);
                    if (!Modifier.isStatic(method.getModifiers()))
                        throw Wlang.makeThrow("Method '%s' of '%s' must be static", name, mirror);
                }
                catch (NoSuchMethodException e) {
                    try {
                        field = mirror.getField(name);
                        if (!Modifier.isStatic(field.getModifiers()))
                            throw Wlang.makeThrow("Field '%s' of '%s' must be static", name, mirror);
                        return;
                    } catch (NoSuchFieldException e1) {
                        throw Wlang.makeThrow("Method or field '%s' don't find in '%s'", name, mirror);
                    }
                }
            } else {
                ...
                if (method == null)
                    throw Wlang.makeThrow("Method '%s' don't find in '%s' or it is NOT static", name, mirror);
                this.args = args;
            }
        }
