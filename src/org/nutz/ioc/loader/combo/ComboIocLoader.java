...
public class ComboIocLoader extends AbstractLifeCycle implements IocLoader {
     ...
     * <p/>
     * 支持类别名: js, json, xml, annotation, anno, trans, async, props, tx,
     * quartz分别对应其加载类
     * <p/>
     * 第二种,为具体的参数
     * <p/>
     ...
     * <p/>
     * 这样的参数,
     * 会生成一个以{"ioc/dao.js","ioc/service.js"}作为参数的JsonLoader,一个以{"ioc/dao.xml"}
    ...
    private void createIocLoader(String className, List<String> args)
        ...
        if (klass == null) {
            if (!className.contains(".")) {
                ...
                _names.add(String.format("org.nutz.integration.%s.%sAopConfigure",
                                         className,
                                         uccp));
                ...
                for (String _className : _names) {
                    klass = (Class<? extends IocLoader>) Wlang.loadClassQuietly(_className);
                    if (klass != null) {
            ...
            if (klass == null)
                klass = (Class<? extends IocLoader>) Wlang.loadClass(className);
        }
