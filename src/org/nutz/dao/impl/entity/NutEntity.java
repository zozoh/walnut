...
public class NutEntity<T> implements Entity<T> {
    ...
    public void checkCompositeFields(String[] names) {
        if (!Wlang.isEmptyArray(names) && names.length > 1) {
            for (String name : names) {
                if (byJava.containsKey(name) && byJava.get(name).isCompositePk())
                    theComposites.add(byJava.get(name));
                else
                    throw Wlang.makeThrow("Fail to find comosite field '%s' in class '%s'!",
                                         name,
                                         type.getName());
            }
    ...
    public void addLinkField(LinkField lnk) {
        ...
        case MANYMANY:
            ...
            break;
        default:
            throw Wlang.makeThrow("It is a miracle in Link field: '%s'(%s)",
                                 lnk.getName(),
    ...
    public List<MappingField> getPks() {
        if (null != theId)
            return Wlang.list(theId);
        if (null != theName)
            return Wlang.list(theName);
