 ...
 * 调用示例
 * <p/>
 * <code>@El("by('net.wendal.util.FuckId')")</code>
 * <p/>
 * <code>@El("by('net.wendal.util.FuckId#staticFunc')")</code>
 * <p/>
 * <code>@El("by('net.wendal.util.FuckId#make')")</code>
 * <p/>
 * 
...
public class ByMake implements RunMethod, Plugin {
    ...
    public Object run(List<Object> fetchParam) {
        ...
        try {
            Class<?> klass = Wlang.loadClass(className);
            if (methodName == null) {
                methodName = "run";
            }
            if (RunMethod.class.isAssignableFrom(klass)) {
                RunMethod it = (RunMethod) klass.getDeclaredConstructor().newInstance();
                return it.run(fetchParam);
            }
            ...
            if (Modifier.isStatic(method.getModifiers())) {
                ...
                Object it = klass.getDeclaredConstructor().newInstance();
