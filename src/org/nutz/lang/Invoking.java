...
public class Invoking {
            ...
            if (null == args || args.length == 0) {
                ...
                for (Iterator<Method> it = candidates.iterator(); it.hasNext();) {
                    ...
                    if (MatchType.YES == mr) {
                        ...
                        invoker = new DefaultInvoker(m, Wlang.arrayLast(    args,
                                                                        Mirror.blankArrayArg(pts)));
                        break;
                    } else if (null != dynaArg && pts.length == 1) {
                        if (pts[0] == dynaArg.getClass()) {
                            invoker = new DynamicArgsInvoker(m, Wlang.array2array(args, pts[0].getComponentType()));
                            break;
                        }
                        if (pts[0].isArray()) {
                    ...
                    try {
                        for (Iterator<Method> it = candidates.iterator(); it.hasNext();) {
                            ...
                            if (pts.length == args.length) {
                                invoker = new DefaultInvoker(m, Wlang.array2ObjectArray(args, pts));
    ...
    public Object invoke(Object obj) {
        try {
            ...
            throw new InvokingException(msg(), Wlang.unwrapThrow(e));
        }
    }
