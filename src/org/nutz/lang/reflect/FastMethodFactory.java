...
public class FastMethodFactory implements Opcodes {
    ...
    protected static FastMethod make(final Method method) {
        ...
        String key = "$FM$" + method.getName() + "$" + Wlang.md5(descriptor);
        String className = ReflectTool.class.getPackage().getName()
                           + "."
                           + Wlang.md5(klass.getName())
                           + key;
        ...
        try {
            fm = (FastMethod) klass.getClassLoader()
                                   .loadClass(className)
        ...
        try {
            byte[] buf = _make(klass,
                               ...
                               className,
                               descriptor);
            Class<?> t = DefaultClassDefiner.defaultOne()
                                            .define(className, buf, klass.getClassLoader());
    ...
    protected static FastMethod make(Constructor<?> constructor) {
        ...
        ;
        String key = Wlang.md5(descriptor);
        String className = ReflectTool.class.getPackage().getName()
                           + "."
                           + Wlang.md5(klass.getName())
                           + "$FC$"
    ...
    /**
     * 生成两种FastClass实例: 创建对象, 和执行普通方法的. 区分的点就是returnType是否为null.
     * 模式为创建对象,returnType总是null, 模式为执行方法,returnType总不是null, 要么Void要么是某个类.
     * 
     * @param klass
     *            被代理的类
     * @param mod
     ...
     *            asm下的方法签名
     * @param returnType
     *            返回值. 如果执行的是构造方法,那么它是null,否则肯定不是null.
     * @param className
     *            目标类名,FastClass实现类的类名
     * @param descriptor
     *            类签名
     */
    public static byte[] _make(Class<?> klass,
        ...
        addConstructor(cw,
                       ...
                       org.nutz.repo.org.objectweb.asm.commons.Method.getMethod("void <init> ()"));

        // 然后定义执行方法, _M就是invoke方法的签名
        GeneratorAdapter mg = new GeneratorAdapter(ACC_PUBLIC,
                                                   _M,
                                                   null,
                                                   new Type[]{Exception_TYPE},
                                                   cw);
        if (returnType == null) { // 没有返回值,是代表这里模式是 "构造方法"
            mg.newInstance(klassType); // 相当于 new User . 注意, 没有括号哦,因为构造方法参数还没传
            mg.dup();
        } else if (!Modifier.isStatic(mod)) { // 有返回值, 那么, 模式是"执行方法". 然后呢,
                                              // 非静态方法的话,需要载入对象
            mg.loadThis();
            mg.loadArg(0); // 代表 Object invoke(Object obj, Object ... args) 中的
                           // obj
            mg.checkCast(klassType); // 因为invoke方法的签名中, obj的类型是Object,
                                     // 需要cast为目标类型.
            // 相当于执行了 ((User)obj)
        }
        ...
        if (params.length > 0) {
            for (int i = 0; i < params.length; i++) {
                mg.loadArg(1); // 代表 Object invoke(Object obj, Object ... args)
                               // 中的 args
                mg.push(i); // 数组下标
                mg.arrayLoad(Type.getType(Object.class)); // 读取数组. 上面三句相当于
                                                          // args[i]
                Type paramType = Type.getType(params[i]); // 读取目标方法/构造方法的参数的类型
                if (params[i].isPrimitive()) {
                    mg.unbox(paramType); // 如果是基本数据类型,需要开箱. Object --> Integer
                                         // --> int .
                                         // 在Java源文件里面有自动封箱/自动开箱,asm可没这种东西.
                } else {
        ...
        if (returnType == null) { // 模式是
            ...
            if (Void.class.equals(returnType)) {
                ...
                // method有返回值,而且是基本数据类型? 那么就要封箱了,因为invoke方法返回的是Object,
                // 基本数据类型可不是Object.
