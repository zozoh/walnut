    ...
    public Map<String, Object> parse(HttpServletRequest req, UploadingContext context)
        ...
        try {
            ...
            if (mm != MarkMode.FOUND) {
                if (log.isWarnEnabled())
                    log.warnf("Fail to find the firstBoundary (%s) in stream, quit!",
                              firstBoundary);
                return params;
            }
            br.skipMark();
        ...
        try {
            ...
            do {
                ...
                if (log.isDebugEnabled())
                    log.debugf("Upload File info: FilePath=[%s],fieldName=[%s]",
                               meta.getFileLocalPath(),
                               meta.getName());
                // 作为文件读取
                if (meta.isFile()) {
                    if (log.isDebugEnabled())
                        log.debugf("Upload Info: name=%s,content_type=%s",
                                   meta.getFileLocalName(),
                                   meta.getContentType());
                    // 检查是否通过文件名过滤
                ...
                // 作为提交值读取
                    ...
                    if (log.isDebugEnabled())
                        log.debugf("Found a param, name=[%s] value=[%s]", meta.getName(), val);
                }
