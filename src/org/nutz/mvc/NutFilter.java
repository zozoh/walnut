...
public class NutFilter implements Filter {
    ...
    public void _init(FilterConfig conf) throws ServletException {
        ...
        if (exclusions != null) {
        	...
        	if (prefix.size() > 0) {
        		exclusionsPrefix = Pattern.compile("^("+Wlang.concat("|", prefix)+")", Pattern.CASE_INSENSITIVE);
        		log.info("exclusionsPrefix  = " + exclusionsPrefix);
        	}
        	if (suffix.size() > 0) {
        		exclusionsSuffix = Pattern.compile("("+Wlang.concat("|", suffix)+")$", Pattern.CASE_INSENSITIVE);
