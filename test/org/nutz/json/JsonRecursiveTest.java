...
public class JsonRecursiveTest {
    ...
    public void test_nutz_json_parents() {
        R r = new R(1);
        r.parents = Wlang.array(new R(-1), new R(-2), new R(-3));
        r.recur = r.parents[1];
    ...
    public void testSimpleRecur() {
        ...
        R r2 = new R(2, r1);
        r1.recur = r2;
        R rr = Json.fromJson(R.class, Wlang.inr(Json.toJson(r1)));
    ...
    public void testSimpleNode() {
        ...
        root.children = Wlang.array(chd1, chd2);
        chd1.children = Wlang.array(chd11, chd12);

        String json = Json.toJson(root, JsonFormat.nice());
        final Node nd = Json.fromJson(Node.class, Wlang.inr(json));
        assertEquals(root.id, nd.id);
        assertEquals(root.children.length, nd.children.length);
        Wlang.each(root.children, (int index, Node ele, Object src) -> {
            assertNull(nd.children[index].parent);
            assertEquals(ele.id, nd.children[index].id);
        });
        Wlang.each(root.children[0].children, (int index, Node ele, Object src) -> {
            assertNull(nd.children[index].parent);
