...
public class WnCronTest {
    ...
    public void test_ext_multi_time_region() {
        WnCron cr;
        // ............................................
        cr = new WnCron("* * ? T[6:00,8:30]{0/30m} T[22:00,22:30]{0/10m}");
    ...
    public void test_ext_normal() {
        WnCron cr;
        // ............................................
        cr = new WnCron("D[20170725,20170921] T[12:23,18:32]{0/30m}");
    ...
    private static void ae_set_part(String cron, String expect, String... parts) {
        WnCron cr = new WnCron().parse(cron);
    ...
    private static void FILL(String[] array,
                             ...
                             String... expect) {
        // 创建+解析
        WnCron cr = new WnCron(rus);

        // 执行
        cr.fill(array, "E", Times.C(ds), off, len, unit);

        // 压缩结果
        List<CronObj<String>> qzos = WnCron.compactAll(array);
    ...
    private static void OVERL(int scale, String ds, String[] russ, String[] expect) {
        ...
        for (String rus : russ) {
            // 创建
            WnCron qz = new WnCron(rus);
            // 执行
            qz.overlapBy(qos, (i++) + ".E", ds);
        }

        // 压缩结果
        List<CronObj<CronOverlapor>> qzos = WnCron.compactAll(qos);
        String[] ss = new String[qzos.size()];
