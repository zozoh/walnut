...
public class WnIoCustomizedPvgTest extends IoCoreTest {
    ...
    public void test_join_strong() {
        ...
        WnUser z = setup.genAccount("zz");

        int pvg;

        pvg = x.getRank(null).evalPvgMode(c.getCustomizedPrivilege(), 0);
        assertEquals(Wn.Io.modeFromOctal("0750"), pvg);

        pvg = y.getRank(null).evalPvgMode(c.getCustomizedPrivilege(), 0);
        assertEquals(0, pvg);

        pvg = z.getRank(null).evalPvgMode(c.getCustomizedPrivilege(), 0);
