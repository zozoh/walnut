...
#===================================================
usage = '''
Walnut 数据迁移程序 (采用 MongoDB.$refs 存储引用)
...
# 调用示例
    ...
    python transdata-ref-mongo.py --limit 5 --skip 10 --flt filter.json
'''
#===================================================
VERSION = "1.0"
def printVersion():
...
phTargetHome = "D:/DB/walnut/io_test/buck"
#
# Mongo 数据库连接
_mongo = {
  "host": "127.0.0.1",
...
print("#" * 60)
print("#")
print("#   Walnut 数据迁移程序(采用 MongoDB.$refs 存储引用)")
print("#  ", VERSION)
print("#")
print("#" * 60)
if flt:
...
for bu in colls:
  ...
  refCollection.update_one(refDoc, {'$setOnInsert': refDoc}, upsert=True)
  #-------------------------------------
  # 准备桶对象，进行转换
  buck = {
    "index": index,
